...
|---|---|---|---|
| `backend/services/execution/__init__.py` | 5 | `execution.*` | **no-op** — `execution/` stays in place |
| `backend/services/execution/shadow_mark_to_market.py` | 1 | `execution.*` | **no-op** — stays |
| `backend/tasks/job_catalog.py` | 1 | `execution.shadow_mark_to_market.run` | **no-op** — stays |
| `backend/tasks/celery_app.py` | 1 | `execution.shadow_mark_to_market` | **no-op** — stays |
| `backend/models/strategy.py` | 2 | `atr_options_strategy`, `dca_strategy` | ⚠ **dead refs** — modules don't exist; flag for cleanup pre-0.C |

Also: all `task="backend.tasks.*"` entries in `job_catalog.py` point at `backend/tasks/`, which is **not touched by 0.C** (services-only scope).

Alembic migrations, frontend code, and YAML configs contain **zero** `backend.services.*` string literals (verified).

...
- Cost-model / unit economics dashboard (`backend/services/billing/provider_costs.py`) — file this as a separate ticket.
...
- Tier plumbing: `Feature` catalog in [`backend/services/billing/feature_catalog.py`](../../backend/services/billing/feature_catalog.py), `require_feature` dep in [`backend/api/dependencies.py`](../../backend/api/dependencies.py) lines 112-146, `TierGate` component on the frontend. Precedent for broker-feature keys: `execution.single_broker` line 188, `execution.multi_broker` line 195.
- Broker account model already has `BrokerType.FIDELITY` ([`backend/models/broker_account.py`](../../backend/models/broker_account.py) line 38). **Do not add `BrokerType.PLAID` as a meta-type** — Plaid is a connection method, not a broker. Instead, add a `connection_source: "direct" | "plaid"` column (see step 1 below). This lets `FIDELITY + plaid` coexist with a future `FIDELITY + direct` without enum thrash.
- Tax lot source enum currently lacks an `AGGREGATOR` value ([`backend/models/tax_lot.py`](../../backend/models/tax_lot.py) line 40-48): add it in the migration below.
- Broker sync registry: [`backend/services/portfolio/broker_sync_service.py`](../../backend/services/portfolio/broker_sync_service.py) lines 138-146 — new factory row for Plaid.

## Alembic migrations (1 revision)

File: `backend/alembic/versions/0075_plaid_integration.py` (parent = `0074_drop_app_settings`).
...
- Create table `plaid_connections`:
  ...
  - `access_token_encrypted TEXT NOT NULL`  — Fernet-encrypted (reuse `backend.services.oauth.encryption`)
...
[`backend/models/plaid_connection.py`](../../backend/models/plaid_connection.py) — mirrors [`backend/models/broker_oauth_connection.py`](../../backend/models/broker_oauth_connection.py) in structure. Register in `backend/models/__init__.py`.

### 2. Update existing model

[`backend/models/broker_account.py`](../../backend/models/broker_account.py): add `connection_source = Column(String(16), nullable=False, default="direct")` and a property `is_aggregator: bool` that returns `connection_source == "plaid"`.

### 3. Update tax-lot enum and guard

[`backend/models/tax_lot.py`](../../backend/models/tax_lot.py):
...
```
backend/services/portfolio/plaid/
  __init__.py
...
[`backend/services/portfolio/broker_sync_service.py`](../../backend/services/portfolio/broker_sync_service.py) around line 138-146: add a new factory. Because `BrokerType` does not include `PLAID`, dispatch based on `BrokerAccount.connection_source == "plaid"` **before** the `BrokerType` switch. Extract a helper `_route_to_service(account)` that takes the `BrokerAccount`, checks `connection_source` first, then falls back to `BrokerType`-based dispatch.

### 6. New API routes

[`backend/api/routes/plaid.py`](../../backend/api/routes/plaid.py), mounted under `/api/v1/plaid`:
...
[`backend/services/billing/feature_catalog.py`](../../backend/services/billing/feature_catalog.py), alongside the existing `execution.*` entries (lines 188+):
...
[`backend/tasks/portfolio/plaid_sync.py`](../../backend/tasks/portfolio/plaid_sync.py):
...
Job catalog entry in [`backend/tasks/job_catalog.py`](../../backend/tasks/job_catalog.py): `plaid_daily_sync`, cron `0 5 * * *` UTC (after overnight FX/settlement), queue `heavy`, `timeout_s=660`. Matches `hard_time_limit` per IRON LAW.

### 9. Health dimension

[`backend/services/monitoring/admin_health_service.py`](../../backend/services/monitoring/admin_health_service.py) (Grep for the existing `dimensions` list): add a `plaid` dimension reporting last successful sync timestamp per user, error counter, and any `needs_reauth` rows. Surfaces in `/admin/health`.
...
| Duplicate sync when webhook and cron race | `PlaidConnection.last_sync_at` + Redis lock per `item_id` (reuse `backend.services.cache.redis_client`) |
...
- Cost model: `backend/services/billing/provider_costs.py` per-provider cost dashboard.

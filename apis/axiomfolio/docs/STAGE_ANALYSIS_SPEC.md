...
> Source of truth: `Stage_Analysis.docx` → code in `backend/services/market/stage_classifier.py`.
...
| File | Purpose |
|------|---------|
| `backend/services/market/stage_classifier.py` | Stage classification logic |
| `backend/services/market/indicator_engine.py` | Indicator computation (SMA, ATR, RSI, etc.) |
| `backend/services/market/regime_engine.py` | Market regime classification (R1–R5) |
| `backend/services/execution/risk_gate.py` | Position sizing + stage/regime caps |
| `backend/services/execution/exit_cascade.py` | 9-tier exit cascade including stage exits |

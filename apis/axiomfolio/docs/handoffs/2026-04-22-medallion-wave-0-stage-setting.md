...
> **For the next chat (Claude Opus, GPT, or any successor — human or agent)**: read this file end-to-end before doing anything in `backend/services/`. It captures *why* we're inserting a Wave 0 ahead of the published 16-wave roadmap, what's actually true on disk vs what the docs claim, and the exact four-phase migration plan with reversibility notes for each phase. The previous Opus session built up this thesis through a meta-architectural audit; do not re-litigate it without reading the receipts in §3.
...
```bash
$ ls backend/services/portfolio/ | grep -E "ibkr|schwab|tastytrade"
ibkr/                              # subdir, ~6 files
...
```bash
$ ls backend/services/silver/ 2>&1
ls: backend/services/silver/: No such file or directory

$ rg "from backend.services.silver" backend/
(no matches)
```

What we call "silver" lives in `backend/services/market/`, `backend/services/portfolio/portfolio_analytics_service.py`, `backend/services/portfolio/closing_lot_matcher.py`, `backend/services/tax/*`. Roughly 25 files, zero docstring tags, zero physical layout.

### 3.3 Gold — 60% real

```bash
$ ls backend/services/gold/
...
7 native gold files. But the older gold-shaped code is in `backend/services/strategy/` (rule evaluator, backtest, walk-forward, signal generator, AI strategy builder) and `backend/services/picks/candidate_generator.py` plus `backend/services/picks/generators/`.

### 3.4 Execution — orthogonal to medallion (currently mis-classified)

```bash
$ ls backend/services/execution/
...
1. Module-level docstring tag on every file under `backend/services/<dir>/`: `"""medallion: bronze"""` / `silver` / `gold` / `execution` / `ops`. ~100 files. `sed`-able. Zero behavior change.
...
1. `backend/services/silver/__init__.py` created with module docstring defining the layer contract.
2. Custom lint rule (Ruff plugin if its plugin API supports import-graph rules; else pylint custom checker via astroid). Enforces:
   - `backend/services/bronze/**` cannot import from `silver/`, `gold/`, `strategy/`, `picks/`, `execution/`.
   - `backend/services/silver/**` cannot import from `gold/`, `strategy/`, `picks/`, `execution/`.
...
1. **Migration script** at `scripts/medallion_migrate.py`:
   ...
   - `git mv` in one pass.
   - Rewrites every `from backend.services.X.Y import ...` and `import backend.services.X.Y` across `backend/`, `tests/`, `alembic/`, `tasks/`.
   - Rewrites `task_path="backend.tasks...."` and `"backend.services...."` strings in `job_catalog.py`, Celery Beat schedules, `agent/tools.py` dotted-path lookups.
   - Updates relative imports.
   - Runs `ruff --fix`, `isort`, `black`.
   - `python -c "import backend.api.main"` to catch import-time failures.
...
3. **Shadow re-exports** for the 2-week bake. Every old location keeps a minimal `__init__.py`:
   ...
   from backend.services.silver.indicator_engine import *  # noqa: F401,F403
   import warnings
   warnings.warn(
       "backend.services.market.indicator_engine is moving to backend.services.silver; "
...
The founder mentioned a parallel agent during the PR #476 session ("there is an agent in parallel but not writing these docs but access to same code"). Wave 0.C is ~55 file renames across `backend/services/`. Any open branch touching `backend/services/market/*` or `backend/services/portfolio/*` during Wave 0.C will have painful rebases.

**Mitigation**: explicit 3-day freeze window. Founder coordinates with the parallel agent: no new PRs against `backend/services/` for the duration of 0.C. Worst case, we rebase-help open branches by running the migration script against them.

**This is decision gate #4** — founder must specify the freeze window before 0.C begins.

### 6.2 Danger-zone approvals

`backend/services/market/indicator_engine.py`, `stage_classifier.py`, `regime_engine.py` are **danger zones** per `.cursor/rules/protected-regions.mdc`. Moving them requires founder approval. The diff is zero-content (just a path change), but approval is still required.
...
**Mitigation**: migration script has explicit coverage for `task_path=` strings + post-migration check that runs `python -c "import backend.tasks.<x>"` for every module listed in `job_catalog.py`. Also: 1-week dual-registration window for Celery tasks (both old and new paths registered).
...
| 4 | 3-day freeze window for Phase 0.C — when? | **TBD by founder** | Parallel agent conflict on `backend/services/` is the only true blocker |
...
If gate #1 = yes:
  1. Confirm freeze window (gate #4) with founder before any backend/services/ changes.
...
- Has the parallel agent stopped touching backend/services/?

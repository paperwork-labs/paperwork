...
Mirrors :mod:`app.models.broker_oauth_connection` in structure:

* Fernet-encrypted access token at rest (see
  :mod:`app.services.oauth.encryption`). Plaintext is never written to
  this column.
...
``docs/KNOWLEDGE.md``. Migration: ``app/alembic/versions/0075_plaid_integration.py``.
"""
...
class PlaidConnection(Base):
    ...
    :func:`app.services.oauth.encryption.decrypt` and must never log

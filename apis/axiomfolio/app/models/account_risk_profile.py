...
``app/services/execution/risk_gate.py``. Effective limits are always
the tighter of ``(firm_cap, per_account_cap)`` — see
``app/services/risk/account_risk_profile.py``.
...
class BrokerAccountRiskProfile(Base):
    ...
    additional constants in ``app/services/risk/firm_caps.py``. The

...
Mounted at ``/api/v1/webhooks/stripe`` (see app/api/routes/webhooks/__init__.py).
...
from app.services.billing.price_catalog import PriceCatalog
from app.services.billing.stripe_client import StripeNotConfigured
...
from app.services.billing.user_resolver import DBUserResolver
...
def _resolve_sink(session: Session) -> SubscriptionStateSink:
    ...
    try:
        from app.services.billing.entitlement_service import (  # type: ignore[import-not-found]
            EntitlementService,
        )
        from app.services.billing.entitlement_sink import (  # type: ignore[import-not-found]

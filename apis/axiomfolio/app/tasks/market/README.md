...
> **Canonical reference** for all `app.tasks.market` tasks—schedules, dependencies, runbooks, and troubleshooting live here.

Celery tasks under `app.tasks.market` drive the **Intelligence** pillar: daily and intraday price ingestion, index universes, indicator recomputation, `MarketSnapshot` / `MarketSnapshotHistory`, coverage health in Redis, regime (R1–R5), fundamentals enrichment, retention, and related maintenance.

Tasks are registered as `app.tasks.market.<module>.<function>`. Most use the `@task_run` decorator for `JobRun` tracking, Redis status keys, single-flight locks, and failure alerts.
...
```python
from app.services.market.market_data_service import provider_router, infra, snapshot_builder
```
...
Positional and keyword arguments are JSON (see [`Makefile`](../../../Makefile) and [`app/scripts/run_task.py`](../../scripts/run_task.py)):

```bash
make task-run TASK=app.tasks.market.coverage.daily_bootstrap \
  TASK_KWARGS='{"history_days":20,"history_batch_size":25}'

make task-run TASK=app.tasks.market.backfill.symbols \
  TASK_ARGS='[["AAPL","MSFT"]]' \
...
- **Celery name** = dotted path `app.tasks.market.<module>.<function>` (this is what `TASK=` uses).

...
def test_render_sync_disabled_when_no_api_key():
    from app.services.core.render_sync_service import RenderCronSyncService

    with patch("app.services.core.render_sync_service.settings") as mock_settings:
...
def test_auto_seed_on_empty_table(db_session):
    ...
    try:
        ...
        with patch("app.api.routes.admin.scheduler._seeded", False):
            resp = client.get("/api/v1/admin/schedules")
        assert resp.status_code == 200

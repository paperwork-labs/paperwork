...
def test_safe_recompute_passes_force_true_and_config_days():
    ...
    with (
        patch("app.tasks.market.backfill._set_task_status"),
        patch(
            "app.tasks.market.indicators.recompute_universe", side_effect=fake_recompute
        ) as mock_rc,
        patch(
            "app.tasks.market.history.snapshot_last_n_days", side_effect=fake_snapshot_last_n
        ) as mock_sn,
        patch("app.tasks.market.coverage.health_check", side_effect=fake_health),
    ):
        from app.tasks.market.backfill import safe_recompute

        result = safe_recompute.__wrapped__(
            since_date="1993-01-29", batch_size=10, history_batch_size=5
        )

...
def test_alembic_head_and_isolation(db_session):
    ...
    db_session.execute(
        text(
            "INSERT INTO users (username, email, password_hash, is_active) VALUES (:u, :e, 'x', true)"
        ),
        {"u": uname, "e": f"{uname}@example.com"},
    )
    res = db_session.execute(
        text("SELECT username FROM users WHERE username=:u"), {"u": uname}
    ).fetchone()

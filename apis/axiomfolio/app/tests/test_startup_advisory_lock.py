...
@patch("app.api.main.SessionLocal")
@patch("app.api.main.engine")
def test_lock_acquired_runs_seed(mock_engine, mock_session_local, mock_settings):
    ...
    with patch("app.scripts.seed_schedules.seed", mock_seed):
        from app.api.main import _sync_deferred_startup
...
def test_lock_released_in_finally(mock_engine, mock_session_local, mock_settings):
    ...
    with patch("app.scripts.seed_schedules.seed", side_effect=RuntimeError("seed kaboom")):
        from app.api.main import _sync_deferred_startup

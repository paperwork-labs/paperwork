...
class TestStage4:
    def test_4c_deep_capitulation(self) -> None:
        ...
        assert _tf(70.0, 100.0, 85.0, 68.0, sma150_slope=-0.5, sma50_slope=-0.4, ext_pct=-30.0) == "4C"
        assert _tf(65.0, 100.0, 85.0, 63.0, sma150_slope=-0.8, sma50_slope=-0.6, ext_pct=-35.0) == "4C"

    def test_4b_accelerating_decline(self) -> None:
        """4B: below, slope strongly down, ext in (-30, -15], SMA50 < SMA150"""
        assert _tf(85.0, 100.0, 95.0, 84.0, sma150_slope=-0.5, sma50_slope=-0.3, ext_pct=-15.0) == "4B"
        assert _tf(80.0, 100.0, 90.0, 78.0, sma150_slope=-0.5, sma50_slope=-0.3, ext_pct=-20.0) == "4B"
    ...
    def test_4a_early_decline(self) -> None:
        ...
        assert _tf(92.0, 100.0, 97.0, 91.0, sma150_slope=-0.5, sma50_slope=-0.3, ext_pct=-8.0) == "4A"
        assert _tf(95.0, 100.0, 97.0, 94.0, sma150_slope=-0.5, sma50_slope=-0.3, ext_pct=-5.0) == "4A"

    def test_4a_flat_slope_with_weak_sma50(self) -> None:
        """4A via flat SMA150 slope + SMA50 slope < -0.35%"""
        assert _tf(93.0, 100.0, 97.0, 92.0, sma150_slope=-0.1, sma50_slope=-0.5, ext_pct=-7.0) == "4A"


class TestStage1:
    def test_1a_deep_base(self) -> None:
        """1A: flat slope, SMA50 <= SMA150, ext in (-5, 0]"""
        assert _tf(98.0, 100.0, 99.0, 97.5, sma150_slope=-0.1, sma50_slope=0.1, ext_pct=-2.0) == "1A"
    ...
    def test_1b_late_base(self) -> None:
        ...
        assert _tf(
            99.0, 100.0, 101.0, 99.5,
            sma21=98.0,
            sma150_slope=0.1, sma50_slope=0.1, ext_pct=-1.0,
        ) == "1B"

    def test_1b_breakout_captured_by_2a(self) -> None:
        """When price breaks above SMA150, ext becomes positive → 2A captures it."""
        assert _tf(
            101.0, 100.0, 95.0, 102.0,
            sma21=97.0,
            sma150_slope=0.2, sma50_slope=0.2, ext_pct=1.0,
            vol_ratio=2.0,
        ) == "2A"
...
class TestStage2:
    ...
    def test_2b_confirmed_advance(self) -> None:
        ...
        assert _tf(
            115.0, 100.0, 108.0, 116.0,
            sma21=113.0,
            sma150_slope=0.5, sma50_slope=0.5, ext_pct=15.0,
        ) == "2B"

    def test_2b_boundary_20_pct(self) -> None:
        """ext_pct = 20.0 is still 2B (boundary inclusive)."""
        assert _tf(
            120.0, 100.0, 110.0, 121.0,
            sma21=118.0,
            sma150_slope=0.6, sma50_slope=0.6, ext_pct=20.0,
        ) == "2B"

    def test_2c_extended(self) -> None:
        """2C: above, slope > 0.35%, ext > 20%"""
        assert _tf(
            125.0, 100.0, 112.0, 126.0,
            sma21=120.0,
            sma150_slope=0.7, sma50_slope=0.6, ext_pct=25.0,
        ) == "2C"
    ...
    def test_2c_via_atre_promoted(self) -> None:
        ...
        r = classify_stage_full(
            close=110.0, sma150=100.0, sma50=105.0, sma21=108.0, ema10=111.0,
            sma150_slope=0.5, sma50_slope=0.5, ext_pct=10.0,
            atre_150=5.0, vol_ratio=1.0,
...
class TestStage3:
    def test_3a_early_distribution(self) -> None:
        ...
        assert _tf(
            99.0, 100.0, 102.0, 98.5,
            sma21=100.0,
            sma150_slope=0.2, sma50_slope=0.1, ext_pct=-1.0,
        ) == "3A"
    ...
    def test_3b_late_distribution(self) -> None:
        ...
        SMA50 > SMA150 to skip 1A disambiguation."""
        assert _tf(
            96.0, 100.0, 101.0, 95.0,
            sma21=97.0,
            sma150_slope=-0.1, sma50_slope=-0.2, ext_pct=-4.0,
        ) == "3B"
...
class TestFallback:
    def test_no_match_returns_3a(self) -> None:
        ...
        result = _tf(
            105.0, 100.0, 104.0, 103.0,
            sma21=103.5,
            sma150_slope=0.5, sma50_slope=0.2, ext_pct=5.0,
        )
        assert result == "3A"

    def test_missing_slopes_returns_prev_stage(self) -> None:
        """Missing slopes → returns prev_stage."""
        assert _tf(100.0, 100.0, 100.0, 100.0, sma150_slope=None, sma50_slope=None, prev_stage="2B") == "2B"
...
class TestClassifyFull:
    def test_atre_promote(self) -> None:
        ...
        r = classify_stage_full(
            close=106.0, sma150=100.0, sma50=103.0, sma21=104.0, ema10=105.0,
            sma150_slope=0.4, sma50_slope=0.4, ext_pct=6.0,
            atre_150=7.0, vol_ratio=1.0,
        )
        assert r.stage_label == "2C"
        assert r.atre_promoted is True
    ...
    def test_pass_count_increments_on_2b_entry(self) -> None:
        ...
        r = classify_stage_full(
            close=115.0, sma150=100.0, sma50=108.0, sma21=110.0, ema10=112.0,
            sma150_slope=0.5, sma50_slope=0.5, ext_pct=15.0,
            atre_150=3.0, vol_ratio=1.0,
            prior_stage="2A", prior_pass_count=0,
        )
    ...
    def test_pass_count_no_increment_staying_2b(self) -> None:
        ...
        r = classify_stage_full(
            ...
            sma150_slope=0.5, sma50_slope=0.5, ext_pct=15.0,
            atre_150=3.0, vol_ratio=1.0,
            prior_stage="2B", prior_pass_count=1,
        )
        assert r.pass_count == 1
    ...
    def test_pass_count_resets_on_stage_4(self) -> None:
        ...
        r = classify_stage_full(
            ...
            sma150_slope=0.5, sma50_slope=0.5, ext_pct=15.0,
            atre_150=3.0, vol_ratio=1.0,
            prior_stage="4A", prior_pass_count=2,
        )
    ...
    def test_second_pass_tracking(self) -> None:
        ...
        r = classify_stage_full(
            ...
            sma150_slope=0.5, sma50_slope=0.5, ext_pct=15.0,
            atre_150=3.0, vol_ratio=1.0,
            prior_stage="2A", prior_pass_count=1,
        )
    ...
    def test_action_override_2c_in_r4(self) -> None:
        ...
        r = classify_stage_full(
            close=125.0, sma150=100.0, sma50=110.0, sma21=118.0, ema10=122.0,
            sma150_slope=0.8, sma50_slope=0.7, ext_pct=25.0,
            atre_150=4.0, vol_ratio=1.0,
            regime_state="R4",
        )
        assert r.stage_label == "2C"
    ...
    def test_manual_review_on_fallback(self) -> None:
        ...
        r = classify_stage_full(
            close=105.0, sma150=100.0, sma50=104.0, sma21=103.5, ema10=103.0,
            sma150_slope=0.5, sma50_slope=0.2, ext_pct=5.0,
            atre_150=2.0, vol_ratio=1.0,
        )
    ...
    def test_genuine_3a_no_manual_review(self) -> None:
        ...
        r = classify_stage_full(
            close=99.0, sma150=100.0, sma50=102.0, sma21=100.0, ema10=98.5,
            sma150_slope=0.2, sma50_slope=0.1, ext_pct=-1.0,
            atre_150=2.0, vol_ratio=1.0,
        )
        assert r.stage_label == "3A"
...
class TestClassifyStageSeries:
    def _run(self, **kw) -> str:
        ...
        result = classify_stage_series(
            data["close"], data["sma150"], data["sma50"], data["sma21"],
            data["ema10"], data["sma150_slope"], data["sma50_slope"],
            data["ext_pct"], data["atre_150"], data["vol_ratio"],
            data["rs_mansfield"],
        )
    ...
    def test_series_4c(self) -> None:
        assert self._run(
            close=65.0, sma150=100.0, sma50=85.0, sma21=70.0, ema10=64.0,
            sma150_slope=-0.8, sma50_slope=-0.6, ext_pct=-35.0,
            atre_150=0.0, vol_ratio=1.0, rs_mansfield=-10.0,
        ) == "4C"

    def test_series_2a(self) -> None:
        assert self._run(
            close=105.0, sma150=100.0, sma50=102.0, sma21=103.0, ema10=104.0,
            sma150_slope=0.3, sma50_slope=0.3, ext_pct=5.0,
            atre_150=2.0, vol_ratio=1.0, rs_mansfield=5.0,
        ) == "2A"

    def test_series_2b(self) -> None:
        assert self._run(
            close=115.0, sma150=100.0, sma50=108.0, sma21=112.0, ema10=114.0,
            sma150_slope=0.5, sma50_slope=0.5, ext_pct=15.0,
            atre_150=3.0, vol_ratio=1.0, rs_mansfield=5.0,
        ) == "2B"
    ...
    def test_series_atre_override(self) -> None:
        ...
        assert self._run(
            close=106.0, sma150=100.0, sma50=103.0, sma21=104.0, ema10=105.0,
            sma150_slope=0.3, sma50_slope=0.3, ext_pct=6.0,
            atre_150=7.0, vol_ratio=1.0, rs_mansfield=5.0,
        ) == "2C"
    ...
    def test_series_breakout_via_2a_rule(self) -> None:
        ...
        assert self._run(
            close=103.0, sma150=100.0, sma50=97.0, sma21=99.0, ema10=101.0,
            sma150_slope=0.2, sma50_slope=0.2, ext_pct=3.0,
            atre_150=1.0, vol_ratio=2.0, rs_mansfield=5.0,
        ) == "2A"
...
class TestExtPctBoundaries:
    def test_ext_neg5_is_4a_not_1a(self) -> None:
        ...
        result = _tf(
            95.0, 100.0, 97.0, 94.0,
            sma150_slope=-0.5, sma50_slope=-0.3, ext_pct=-5.0,
        )
        assert result == "4A"
    ...
    def test_ext_neg4_99_is_1a_zone(self) -> None:
        ...
        result = _tf(
            95.01, 100.0, 98.0, 94.0,
            sma150_slope=-0.1, sma50_slope=0.1, ext_pct=-4.99,
        )
        assert result == "1A"
    ...
    def test_ext_0_is_1a_with_sma50_below(self) -> None:
        ...
        result = _tf(
            ...
            sma150_slope=0.0, sma50_slope=0.0, ext_pct=0.0,
        )
        assert result == "1A"
    ...
    def test_ext_8_01_is_2b(self) -> None:
        ...
        result = _tf(
            108.01, 100.0, 105.0, 109.0,
            sma21=107.0,
            sma150_slope=0.5, sma50_slope=0.5, ext_pct=8.01,
        )
        assert result == "2B"
    ...
    def test_ext_20_01_is_2c(self) -> None:
        """ext_pct > 20 is 2C."""
        result = _tf(
            120.01, 100.0, 110.0, 121.0,
            sma21=118.0,
            sma150_slope=0.6, sma50_slope=0.6, ext_pct=20.01,
        )
        assert result == "2C"
    ...
    def test_ext_neg30_is_4c(self) -> None:
        ...
        result = _tf(
            70.0, 100.0, 85.0, 68.0,
            sma150_slope=-0.5, sma50_slope=-0.4, ext_pct=-30.0,
        )
        assert result == "4C"
    ...
    def test_ext_neg29_99_is_4b(self) -> None:
        ...
        result = _tf(
            70.01, 100.0, 90.0, 68.0,
            sma150_slope=-0.5, sma50_slope=-0.3, ext_pct=-29.99,
        )
        assert result == "4B"

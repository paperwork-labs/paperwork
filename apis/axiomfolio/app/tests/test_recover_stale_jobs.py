...
def test_only_stale_running_jobs_are_cancelled(db_session):
    ...
    fresh = JobRun(task_name="task_b", status="running", started_at=now - timedelta(minutes=30))
        ...
        task_name="task_c", status="ok", started_at=now - timedelta(hours=5), finished_at=now
    )

    db_session.add_all([stale, fresh, done])
    db_session.flush()

    from app.tasks.market.maintenance import recover_jobs_impl

    with patch("app.tasks.market.maintenance.SessionLocal", return_value=db_session):

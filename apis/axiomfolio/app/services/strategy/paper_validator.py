...
class PaperValidator:
    ...
    def __init__(self, db: Session, config: ValidationConfig | None = None):
        self.db = db
    ...
    def check_validation(
        ...
        days_elapsed = (datetime.now(UTC) - started_at).days
    ...
    def _calculate_metrics(self, orders: list[Order]) -> dict[str, Any]:
        ...
        for order in orders:
            ...
            if order.filled_avg_price and order.filled_quantity:
                notional = Decimal(str(order.filled_avg_price)) * Decimal(
            ...
            if pnl > 0:
                ...
                if order.cost_basis and order.cost_basis > 0 and order.filled_quantity:
                    trade_cost = Decimal(str(order.cost_basis)) * Decimal(
                        str(order.filled_quantity)
                    )
                    if trade_cost > 0:
    ...
    def promote_to_live(
        ...
        metadata = strategy.audit_metadata or {}
        metadata["paper_validation"]["completed_at"] = datetime.now(UTC).isoformat()

...
* **Gold** (``app/services/gold/*``) — decision services:
  PickQualityScorer, OptionsChainSurface, TradeCardComposer, RiskGate,
  OrderManager.

Existing modules under ``app/services/portfolio/ibkr``,
``app/services/portfolio/schwab_sync_service.py`` and
``app/services/portfolio/tastytrade_sync_service.py`` are bronze-layer

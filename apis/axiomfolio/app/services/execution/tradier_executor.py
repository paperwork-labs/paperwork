...
:mod:`app.services.execution.oauth_executor_mixin` and the extended
Tradier HTTP helpers from :mod:`app.services.bronze.tradier.client`.
...
* Per-call DB + connection load. Tradier is multi-user: each order is
  ...
  :class:`app.models.order.Order` row keyed by ``broker_order_id``.

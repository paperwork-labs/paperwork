...
class OrderManager:
    ...
    async def preview(
        ...
        """
        price = self.risk_gate.estimate_price(db, req.symbol, req.limit_price, req.stop_price)
    ...
    def _get_portfolio_equity(self, db: Session, user_id: int) -> float | None:
    ...
    async def submit(
        ...
        try:
            ...
            lock_acquired = redis_client.set(lock_key, "1", nx=True, ex=SUBMIT_LOCK_TTL_SECONDS)
            ...
            logger.warning(
                "Redis lock failed for order %s, proceeding with DB check: %s", order_id, e
            )
        ...
        price = self.risk_gate.estimate_price(db, req.symbol, req.limit_price, req.stop_price)
        risk_budget = portfolio_equity * 0.01 if portfolio_equity and portfolio_equity > 0 else None
        try:
    ...
    async def poll_status(
        ...
        if not order:
            return {"error": "Order not found"}
        if not order.broker_order_id or order.status in (
        ...
        if new_status == OrderStatus.FILLED.value and not order.filled_at:
            ...
            try:
                ...
                logger.warning(
                    "Failed to record fill with circuit breaker for order %s: %s", order.id, e
                )
    ...
    def list_orders(
        ...
        list_source: Literal["all", "app", "broker"] | str = "all",
        ...
        order_account_id_str: str | None = None
        scoped_broker_account_id: int | None = None
        ...
        order_sort = func.coalesce(Order.filled_at, Order.submitted_at, Order.created_at)
        trade_sort = func.coalesce(Trade.execution_time, Trade.order_time, Trade.created_at)
        ...
        if src == "app":
            ...
            res = (
                db.execute(sel_app.order_by(order_sort.desc().nulls_last()).offset(off).limit(cap))
                .mappings()
                .all()
            )
        ...
        if src == "broker":
            sel_br = (
                select(Trade.id.label("trade_id"), trade_sort.label("sort_ts"))
                .select_from(Trade)
            ...
            res = (
                db.execute(sel_br.order_by(trade_sort.desc().nulls_last()).offset(off).limit(cap))
                .mappings()
                .all()
            )
        ...
        page_q = select(union_subq).order_by(union_subq.c.sort_ts.desc().nulls_last())
        if off:
        ...
        by_oid = {o.id: o for o in db.query(Order).filter(Order.id.in_(oids)).all()} if oids else {}
        if tids:
            trade_pairs = (
...
def _trade_to_ledger_dict(t: Trade, acct: BrokerAccount | None, user_id: int) -> dict[str, Any]:
    ...
    return {
        "id": -int(
            t.id
        ),  # synthetic: negative ids are broker-ledger; no collision with live orders
        "symbol": t.symbol,

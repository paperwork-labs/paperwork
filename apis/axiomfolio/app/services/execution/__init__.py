...
def __getattr__(name: str) -> Any:
    if name in ("BrokerExecutor", "OrderRequest", "OrderResult"):
        m = importlib.import_module("app.services.execution.broker_base")
        return getattr(m, name)
    if name == "BrokerRouter":
        return importlib.import_module("app.services.execution.broker_router").BrokerRouter
    if name in ("RiskGate", "RiskViolation"):
        m = importlib.import_module("app.services.execution.risk_gate")
        return getattr(m, name)
    if name == "OrderManager":
        return importlib.import_module("app.services.execution.order_manager").OrderManager
    if name == "PaperExecutor":
        return importlib.import_module("app.services.execution.paper_executor").PaperExecutor

...
from app.models import BrokerAccount, Trade, Transaction
from app.models.position import Position
from app.models.transaction import Dividend, TransactionSyncStatus, TransactionType
from app.services.clients.ibkr_flexquery_client import IBKRFlexQueryClient

from .helpers import logger, serialize_for_json
    ...
    try:
        ...
        def _store_trade(td: dict, status: str) -> None:
            ...
            values = dict(
                ...
                execution_time=td.get("execution_time") or datetime.now(UTC),
    ...
    try:
        ...
        if not transactions_data:
            ...
            return {
                "synced": 0,
                "dividends": 0,
                "note": "FlexQuery returned no CashTransactions section",
            }
        ...
        for tx_data in transactions_data:
            try:
                ...
                logger.error(
                    "Error processing cash transaction %s: %s", tx_data.get("external_id", "?"), exc
                )
                continue
        ...
        db.flush()
        logger.info(
            "Cash transactions: %d transactions, %d dividends", synced_count, dividend_count
        )
        return {
            "synced": synced_count,
...
def _enrich_dividend_frequency(db: Session, broker_account: BrokerAccount) -> None:
    ...
    for sym, divs in divs_by_symbol.items():
        ...
        day_diffs = [(divs[i].ex_date - divs[i - 1].ex_date).days for i in range(1, len(divs))]
        if not day_diffs:
            continue

...
from backend.models.market_data import EarningsCalendarEvent, MarketRegime, MarketSnapshot
from backend.services.market.constants import CURATED_MARKET_SYMBOLS
from backend.services.market.indicator_engine import compute_full_indicator_series, extract_latest_values
from backend.services.market.market_data_service import price_bars, snapshot_builder
from backend.services.market.snapshot_builder import next_earnings_utc_from_calendar
...
def test_next_earnings_utc_today_eod_stays_future_dated(db_session, monkeypatch):
    ...
    monkeypatch.setattr(
        "backend.services.market.snapshot_builder.datetime",
        _FauxDateTime,
    )
    d_today = fixed.date()
    db_session.add(
...
def test_earnings_calendar_exception_preserves_next_earnings_and_increments_errors(
    ...
    monkeypatch.setattr(
        "backend.services.market.snapshot_builder.next_earnings_utc_from_calendar",
        _raise,
    )
    m: dict[str, int] = {}
    snap = snapshot_builder.compute_snapshot_from_db(
...
def test_run_scan_overlay_writes_regime_state_from_market_regime(db_session, monkeypatch):
    from backend.tasks.market import coverage

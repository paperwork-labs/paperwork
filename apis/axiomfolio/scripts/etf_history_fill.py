...
def compute_and_insert(session, symbol: str, missing_dates: list[datetime], dry_run: bool = False):
    ...
    for idx in df.index:
        ...
        row_data = {
            ...
            if "Volume" in df.columns and pd.notna(df.loc[idx, "Volume"])
            else None,
            "sma_50": float(indicators.get("sma_50", pd.Series()).get(idx))
            if idx in indicators.get("sma_50", pd.Series()).index
            else None,
            "sma_150": float(indicators.get("sma_150", pd.Series()).get(idx))
            if idx in indicators.get("sma_150", pd.Series()).index
            else None,
            "sma_200": float(indicators.get("sma_200", pd.Series()).get(idx))
            if idx in indicators.get("sma_200", pd.Series()).index
            else None,
            "rsi_14": float(indicators.get("rsi_14", pd.Series()).get(idx))
            if idx in indicators.get("rsi_14", pd.Series()).index
            else None,
            "atr_14": float(indicators.get("atr_14", pd.Series()).get(idx))
            if idx in indicators.get("atr_14", pd.Series()).index
            else None,
            "stage": stage_info.get("stage", pd.Series()).get(idx)
            if idx in stage_info.get("stage", pd.Series()).index
            else "UNKNOWN",
            "rs_line": float(rs_line.get(idx))
            if rs_line is not None and idx in rs_line.index and pd.notna(rs_line.get(idx))
            else None,

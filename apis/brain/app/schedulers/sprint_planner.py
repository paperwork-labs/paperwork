...
posts to Brain Conversations (tag: sprint-planning), and optionally appends a snapshot to
...
_STRATEGY_CHANNEL_ID = "C0AM2310P8A"  # kept for agent.process channel_id routing hint
...
def build_sprint_planning_prompt(
    ...
    lines.extend(
        [
            "",
            "Respond with well-structured markdown for the strategy conversation: "
...
async def _append_knowledge_snapshot(body: str) -> None:
...
async def _run_sprint_planner_body() -> None:
    ...
    create_conversation(
        ConversationCreate(
            title=f"Sprint Planning Brief — {date_s}",
            body_md=out,

...
def _priority_weight(path: Path, body: str) -> int:
    ...
    if m:
        p = m.group(1).upper()
        return {"P0": 0, "P1": 1, "P2": 2, "P3": 3, "HIGH": 0, "MEDIUM": 2, "LOW": 4}.get(
            p, 5
        )
...
def build_sprint_planning_prompt(
    ...
    for r in inprog:
        ...
        pct = int(round(100 * r.done_todos / total)) if total else 0
        lines.append(
    ...
    for r in buckets["archive_candidates"][:8]:
        lines.append(
            f"- **{r.title}** (ended {r.end_date}) — consider moving narrative to KNOWLEDGE / archive"
        )
    if not buckets["archive_candidates"]:
        lines.append("- _(none)_")

    open_ip = [r for r in inprog]
    if open_ip:
...
async def _run_sprint_planner_body() -> None:
    request_id = f"sprint-planner:brain:{datetime.now(timezone.utc).isoformat()}"
    root = _repo_root()
...
def install(scheduler: AsyncIOScheduler) -> None:
    """Register Monday 14:00 America/Los_Angeles when :envvar:`BRAIN_OWNS_SPRINT_PLANNER` is true."""

...
_ROUTING_AFTER = re.compile(
    r"\b\d{9}\b(?=(?:\s+\w+){0,2}\s*\b(?:routing|aba|bank)\b)", re.IGNORECASE
)
_ROUTING_BEFORE = re.compile(
    r"\b(?:routing|aba|bank)\b(?:\s+\w+){0,2}\s*\b(\d{9})\b", re.IGNORECASE
)

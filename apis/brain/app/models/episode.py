...
from sqlalchemy import DateTime, Float, Integer, SmallInteger, Text, func
...
class Episode(Base):
    ...
    visibility: Mapped[str] = mapped_column(Text, server_default="organization")
    ...
    embedding_model: Mapped[str] = mapped_column(Text, server_default="text-embedding-3-small")
    importance: Mapped[float] = mapped_column(Float, server_default="0.5")
    freshness: Mapped[float] = mapped_column(Float, server_default="1.0")

...
def test_dispatch_entry_defaults() -> None:
    e = DispatchEntry(task_id="t-1", source="probe")
    assert e.status == "pending"
    assert e.agent_model == "cheap"
...
def test_cheap_model_for_simple_task() -> None:
    ...
    assert select_agent_model(row) == "cheap"
...
def test_expensive_model_for_complex_task() -> None:
    ...
    assert select_agent_model(row) == "expensive"
...
def test_dispatch_entry_creates_pair() -> None:
    ...
    assert entry.agent_model == "cheap"

/**
 * Washington Portal Handler
 *
 * Tier 2 portal automation for Washington Secretary of State CCFS.
 * Extends BasePortalHandler with WA-specific field mapping.
 */
...
export class WashingtonPortalHandler extends BasePortalHandler {
  readonly supportedStates = ["WA"];
  protected readonly config: PortalConfig;

  constructor() {
    super();
    this.config = loadPortalConfig("WA");
  }

  canHandle(stateCode: string): boolean {
    return stateCode.toUpperCase() === "WA";
  }

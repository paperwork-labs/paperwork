/**
 * Nevada Portal Handler
 *
 * Tier 2 portal automation for Nevada Secretary of State SilverFlume.
 * Extends BasePortalHandler with NV-specific field mapping.
 *
 * NV includes a members-list step in the portal; field mapping matches the shared approach.
 */
...
export class NevadaPortalHandler extends BasePortalHandler {
  readonly supportedStates = ["NV"];
  protected readonly config: PortalConfig;

  constructor() {
    super();
    this.config = loadPortalConfig("NV");
  }

  canHandle(stateCode: string): boolean {
    return stateCode.toUpperCase() === "NV";
  }

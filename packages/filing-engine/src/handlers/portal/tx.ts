/**
 * Texas Portal Handler
 *
 * Tier 2 portal automation for Texas Secretary of State SOSDirect.
 * Certificate of Formation (field mapping aligns with our Articles-style schema).
 */
...
export class TexasPortalHandler extends BasePortalHandler {
  readonly supportedStates = ["TX"];
  protected readonly config: PortalConfig;

  constructor() {
    super();
    this.config = loadPortalConfig("TX");
  }

  canHandle(stateCode: string): boolean {
    return stateCode.toUpperCase() === "TX";
  }

/**
 * New York Portal Handler
 *
 * Tier 2 portal automation for New York Department of State.
 * Extends BasePortalHandler with NY-specific field mapping.
 *
 * NY LLC publication requirement is post-filing compliance, not part of portal automation.
 */
...
export class NewYorkPortalHandler extends BasePortalHandler {
  readonly supportedStates = ["NY"];
  protected readonly config: PortalConfig;

  constructor() {
    super();
    this.config = loadPortalConfig("NY");
  }

  canHandle(stateCode: string): boolean {
    return stateCode.toUpperCase() === "NY";
  }

/**
 * Georgia Portal Handler
 *
 * Tier 2 portal automation for Georgia Secretary of State.
 * Extends BasePortalHandler with GA-specific field mapping.
 */
...
export class GeorgiaPortalHandler extends BasePortalHandler {
  readonly supportedStates = ["GA"];
  protected readonly config: PortalConfig;

  constructor() {
    super();
    this.config = loadPortalConfig("GA");
  }

  canHandle(stateCode: string): boolean {
    return stateCode.toUpperCase() === "GA";
  }

#!/usr/bin/env python3
"""Trigger Brain's decision-doc ingestion (``doc_kind: decision``, ``DECISIONS.md``, etc.).

POSTs ``/admin/ingest-decisions``. Requires ``BRAIN_API_SECRET``."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Exit 0 if secret is set; no HTTP call",
    )
    ...
    url = args.brain_url.rstrip("/") + "/admin/ingest-decisions"
    ...
    try:
        ...
        print(f"error: brain returned HTTP {e.code}: {e.read().decode()}", file=sys.stderr)
        return 1

...
function AccountRow({
  ...
  const isErrored =
    account.sync_status &&
    ["error", "failed"].includes(String(account.sync_status).toLowerCase());
        ...
        <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-2">
              <h3 className="truncate font-heading text-sm font-medium text-foreground">
                {account.account_name?.trim() ||
                  `${brokerLabel(account.broker)} · ${account.account_number}`}
              </h3>
              <Button
                type="button"
                size="icon-xs"
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            ...
            {isErrored ? (
              <Badge
                variant="destructive"
                className={cn("h-4 text-[10px]")}
                title={account.sync_error_message ?? undefined}
              >
                Sync error
              </Badge>
            ) : null}
        ...
        <div className="flex shrink-0 items-center gap-2">
          {showSyncHistory ? (
            <Button asChild type="button" size="sm" variant="outline">
              <Link to="/settings/admin/system">Sync history</Link>
            </Button>
          ) : null}
          <Button
            type="button"
...
export default function AccountsManagement() {
  ...
  const [confirmDisconnectId, setConfirmDisconnectId] = React.useState<number | null>(
    null,
  );
            ...
            <AccountRow
              ...
              isMutating={
                (renameMutation.isPending &&
                  renameMutation.variables?.id === account.id) ||
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              We&apos;ll stop syncing and remove the credentials. Historical positions
              and trades stay in your portfolio. You can reconnect at any time.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button

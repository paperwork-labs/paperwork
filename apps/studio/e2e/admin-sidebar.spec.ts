...
test.describe("Admin sidebar (E2E — STUDIO_E2E_FIXTURE=1 dev server)", () => {
  test("WS-76 PR-26: Money + Trackers + Trust + Brain, Bills/Vendors/Cost monitor, 22 nav links, 6 vendor footer links", async ({
    page,
  }) => {
    ...
    // Drawer copy is aria-hidden/inert when closed; `.first()` matches it → 0 role=link hits.

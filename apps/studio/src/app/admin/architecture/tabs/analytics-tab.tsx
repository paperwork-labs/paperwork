// Content moved from /admin/analytics (WS-69 PR C). The /admin/analytics route
  ...
  return (
    <div className="space-y-6">
      <p className="text-zinc-400">
        PostHog visibility for growth, activation, and conversion tracking.
      </p>
      ...
      {dashboardUrls.length > 0 ? (
        ...
        <section className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-5">
          <p className="text-sm text-zinc-300">
            Add a shared dashboard URL in{" "}
            <code className="rounded bg-zinc-800 px-1 font-mono text-xs">
              POSTHOG_DASHBOARD_URL
            </code>{" "}
            to embed analytics here. Multiple dashboards are supported with comma-separated URLs.
          </p>
          <ul className="mt-3 space-y-2 text-sm text-zinc-400">
            {config.map((item) => (
              <li key={item.key}>
                <span className="font-mono text-xs text-zinc-300">{item.key}</span>:{" "}
                {item.value ? (

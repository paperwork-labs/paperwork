...
import { ProductBreadcrumbSwitcher } from "@/components/admin/ProductBreadcrumbSwitcher";
        ...
        <nav aria-label="Breadcrumb" className="text-xs text-zinc-400">
          <ol className="flex flex-wrap items-center gap-1.5">
            {breadcrumbs.map((crumb, i) => {
              const key =
                "productSwitcher" in crumb
                  ? `switcher-${crumb.productSwitcher.currentSlug}-${i}`
                  : `${crumb.label}-${i}`;
              return (
                <li key={key} className="flex items-center gap-1.5">
                  {i > 0 ? <span className="text-zinc-500" aria-hidden>/</span> : null}
                  {"productSwitcher" in crumb ? (
                    <ProductBreadcrumbSwitcher
                      currentSlug={crumb.productSwitcher.currentSlug}
                      options={crumb.productSwitcher.options}
                    />
                  ) : crumb.href ? (
                    <Link
                      href={crumb.href}

...
export default function SettingsUsersClient() {
  ...
  const inviteUser = async () => {
    ...
    try {
      ...
      const url = token ? `${typeof window !== "undefined" ? window.location.origin : ""}/invite/${token}` : null;
      setInviteUrl(url);
      setInviteEmail("");
      await load();
                        ...
                        <td className="px-3 py-2">
                          <div className="flex flex-wrap items-center gap-2">
                            {isPendingApproval ? (
                              ...
                              >
                                {approvingUserId === u.id ? <Loader2 className="size-3 animate-spin" aria-hidden /> : null}
                                Approve
                              </Button>
                            ) : null}
                            <Button
                ...
                <table className="w-full border-collapse text-sm">
                  ...
                  <tbody>
                    {invites
                      ...
                      .map((i) => {
                        const link = i.token
                          ? `${typeof window !== "undefined" ? window.location.origin : ""}/invite/${i.token}`
                          : "";
                        const isExpired = new Date(i.expires_at) < new Date();
                        return (
                          <tr
                            key={i.id}
                            className={cn("border-b border-border last:border-0", isExpired && "opacity-50")}
                          >
      ...
      <Dialog open={Boolean(deleteTarget)} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <DialogContent showCloseButton className="max-w-[400px]">
          ...
          <p className="text-sm text-foreground">
            Are you sure you want to delete <strong>{deleteTarget?.full_name || deleteTarget?.email}</strong>? This action
            cannot be undone.
          </p>
          <DialogFooter>

...
const SCHWAB_LOGO = "/broker-logos/schwab.svg";
const TASTYTRADE_LOGO = "/broker-logos/tastytrade.svg";
const IBKR_LOGO = "/broker-logos/ibkr.svg";
const ETRADE_LOGO = "/broker-logos/etrade.svg";
const TRADIER_LOGO = "/broker-logos/tradier.svg";
const COINBASE_LOGO = "/broker-logos/coinbase.svg";
const IB_GATEWAY_LOGO = "/broker-logos/ib-gateway.svg";
const TRADINGVIEW_LOGO = "/broker-logos/tradingview.svg";
const FMP_LOGO = "/broker-logos/fmp.svg";
...
function legacyToast(args: {
  title: string;
  description?: string;
  status?: "success" | "error" | "info" | "warning";
}) {
  const msg = args.description ? `${args.title}: ${args.description}` : args.title;
  if (args.status === "success") return toast.success(args.title);
  if (args.status === "error") return toast.error(msg);
  return toast(msg);
}
...
export default function SettingsConnectionsClient() {
  ...
  const loadAccounts = async () => {
    try {
      ...
      legacyToast({ title: 'Load accounts failed', description: handleApiError(e), status: 'error' });
    }
  };
  ...
  useEffect(() => {
    ...
    if (schwabStatus === 'linked') {
      legacyToast({ title: 'Schwab account linked successfully', status: 'success' });
      ...
      legacyToast({ title: 'Schwab linking failed', description: reason.replace(/_/g, ' '), status: 'error' });
      window.history.replaceState({}, '', window.location.pathname);
    }
    ...
    if (etradeStatus === 'linked') {
      legacyToast({ title: 'E*TRADE account linked successfully', status: 'success' });
      loadAccounts();
      loadSyncHistory();
      window.history.replaceState({}, '', window.location.pathname);
    }
    ...
    if (tradierStatus === 'linked' && tradierCode && tradierState) {
      ...
      (async () => {
        ...
        try {
          ...
          legacyToast({ title: 'Tradier connected', status: 'success' });
          await loadAccounts();
    ...
    if (coinbaseStatus === 'linked' && coinbaseCode && coinbaseState) {
      (async () => {
        ...
        try {
          ...
          legacyToast({ title: 'Coinbase connected', status: 'success' });
          await loadAccounts();
          await loadSyncHistory();
          notifyOtherTabs();
        } catch (e) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      legacyToast({ title: 'Account added', status: 'success' });
    } catch (e) {
      legacyToast({ title: 'Add account failed', description: handleApiError(e), status: 'error' });
    } finally {
      setAdding(false);
    }
  };
  ...
  const handleConnectSchwab = async (id: number) => {
    ...
    try {
      if (cfg && !cfg.schwabConfigured) {
        ...
        legacyToast({ title: 'Schwab OAuth not configured', description: 'Ask admin to set client_id, secret, and redirect URI on the server.', status: 'warning' });
        return;
      }
      ...
      if (url) {
        ...
        legacyToast({ title: 'Complete Schwab connect in the new tab', status: 'info' });
      }
    } catch (e) {
      try {
        popup?.close();
      ...
      legacyToast({ title: 'Link failed', description: handleApiError(e), status: 'error' });
    }
  };
  ...
  const handleSync = async (id: number) => {
    try {
      ...
      if (res?.task_id || res?.status) {
        pollSyncStatus(id);
        legacyToast({ title: 'Sync started', status: 'success' });
      }
    } catch (e) {
      legacyToast({ title: 'Sync failed', description: handleApiError(e), status: 'error' });
    }
  };
    ...
    try {
      ...
      try {
        ...
        if (s0?.connected) {
          ...
          legacyToast({ title: 'Tastytrade connected', status: 'success' });
          await loadAccounts();
          return;
        }
      } catch { /* ignore */ }
      ...
      legacyToast({ title: 'Connect failed', description: handleApiError(e), status: 'error' });
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleTTDisconnect = async () => {
    try {
      ...
      legacyToast({ title: 'Tastytrade disconnected', status: 'success' });
      await loadAccounts();
    } catch (e) {
      legacyToast({ title: 'Disconnect failed', description: handleApiError(e), status: 'error' });
    }
  };
  ...
  const handleEtradeAuthorize = async () => {
    ...
    try {
      ...
      legacyToast({ title: 'E*TRADE authorize failed', description: handleApiError(e), status: 'error' });
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleCoinbaseAuthorize = async () => {
    ...
    try {
      ...
      try { popup?.close(); } catch { /* noop */ }
      legacyToast({ title: 'Coinbase authorize failed', description: handleApiError(e), status: 'error' });
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleTradierAuthorize = async (env: 'sandbox' | 'live') => {
    ...
    try {
      ...
      try { popup?.close(); } catch { /* noop */ }
      legacyToast({ title: 'Tradier authorize failed', description: handleApiError(e), status: 'error' });
    } finally {
      setBusy(false);
    }
  };
  ...
  const submitWizard = async () => {
    try {
      ...
      if (broker === 'SCHWAB') {
        ...
        legacyToast({ title: 'E*TRADE connected', status: 'success' });
        await loadAccounts();
        await loadSyncHistory();
        ...
        legacyToast({ title: 'Tradier connection in progress', status: 'info' });
      } else if (broker === 'COINBASE') {
        if (!coinbaseForm.state) {
          throw new Error('Click Authorize first to complete Coinbase OAuth');
        }
        legacyToast({ title: 'Coinbase connection in progress', status: 'info' });
      } else if (broker === 'IBKR') {
        ...
        try {
          ...
          if (s0?.connected || (Array.isArray(s0?.accounts) && s0.accounts.length > 0)) {
            legacyToast({ title: 'Interactive Brokers connected', status: 'success' });
            await loadAccounts();
            setWizardOpen(false);
            return;
          }
      ...
      legacyToast({ title: 'Connection failed', description: handleApiError(e), status: 'error' });
    } finally {
      setBusy(false);
    }
  };
  ...
  const gatewayConnectMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('Gateway reconnection triggered — auto-checking status...');
      let polls = 0;
    ...
    onError: (err: unknown) => { toast.error(`Gateway connect failed: ${handleApiError(err)}`); },
  });
  ...
  const saveGwSettings = useMutation({
    ...
    onError: (err: unknown) => { toast.error(`Save failed: ${handleApiError(err)}`); },
  });
  ...
  const tvPrefsMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('TradingView preferences saved');
    },
    onError: (err: unknown) => { toast.error(`Save failed: ${handleApiError(err)}`); },
  });
  ...
  const syncAllMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Sync queued for all enabled accounts');
      void loadAccounts();
      void loadSyncHistory();
      void queryClient.invalidateQueries({ queryKey: ['connectionsHealth'] });
    },
    onError: (err: unknown) => {
      toast.error(`Sync failed: ${handleApiError(err)}`);
    },
  });
          ...
          {showSetupBanner && accounts.filter(a => String(a.broker).toUpperCase() === 'SCHWAB').length > 0 && (
            <div className="rounded-xl border-2 border-[rgb(var(--status-success))] bg-muted/40 p-4">
                    ...
                    <select
                      ...
                      onChange={async (e) => {
                        try {
                          ...
                          legacyToast({ title: 'Update failed', status: 'error', description: handleApiError(err) });
                        }
                      }}
                    >
              ...
              <div className="flex flex-row items-center gap-2">
                <Button type="button" size="sm" onClick={() => { setShowSetupBanner(false); router.push("/portfolio"); }}>
                  Go to Portfolio
                </Button>
          ...
          */}
          ...
          {detailWizardBroker ? (
            <ConnectionDetailSheet
              ...
              onToggleTrack={async (accountId, next) => {
                ...
                await loadAccounts();
                legacyToast({ title: next ? 'Tracking in portfolio' : 'Removed from portfolio', status: 'success' });
              }}
          ...
          >
            <div className="flex flex-col gap-3 items-stretch">
              <div className="mb-1 flex flex-row items-center gap-3">
                <img src={IB_GATEWAY_LOGO} alt="IB Gateway" className="size-7 rounded-md" />
          ...
          <AppCard className="border-l-[3px] border-l-[rgb(var(--status-success))]">
            <div className="flex flex-col gap-4 items-stretch">
              <div className="mb-1 flex flex-row items-center gap-3">
                <img src={TRADINGVIEW_LOGO} alt="TradingView" className="size-7 rounded-md" />
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="rounded-lg border border-border p-4 opacity-60">
                  <div className="mb-2 flex flex-row items-center gap-3">
                    <img src={FMP_LOGO} alt="FMP" className="size-6 rounded-md" />
            ...
            <Button
              ...
              onClick={async () => {
                ...
                if (broker === 'tastytrade') {
                  ...
                  if (!hasAny) {
                    legacyToast({ title: 'Enter at least one credential to update', status: 'error' });
                    return;
                  }
                } else if (broker === 'ibkr') {
                ...
                try {
                  ...
                  legacyToast({ title: 'Credentials updated', status: 'success' });
                  setEditOpen(false);
                  setEditAccount(null);
                  await loadAccounts();
                } catch (e) {
                  legacyToast({ title: 'Update failed', description: handleApiError(e), status: 'error' });
                } finally {
                  setBusy(false);
                }
              }}
            ...
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  legacyToast({ title: 'Account deleted', status: 'success' });
                  await loadAccounts();
                } catch (e) {
                  legacyToast({ title: 'Delete failed', description: handleApiError(e), status: 'error' });
                } finally {
            ...
            {step === 1 && (
              <div className="flex flex-col gap-4 items-stretch">
                ...
                <div className="grid grid-cols-2 gap-6 md:grid-cols-4">
                  <LogoTile label="Charles Schwab" srcs={[SCHWAB_LOGO]} selected={broker === 'SCHWAB'} onClick={() => { setBroker('SCHWAB'); setStep(2); }} wide />
                  <LogoTile label="Tastytrade" srcs={[TASTYTRADE_LOGO]} selected={broker === 'TASTYTRADE'} onClick={() => { setBroker('TASTYTRADE'); setStep(2); }} wide />
                  <LogoTile label="Interactive Brokers" srcs={[IBKR_LOGO]} selected={broker === 'IBKR'} onClick={() => { setBroker('IBKR'); setStep(2); }} wide />
                  <LogoTile label="E*TRADE" srcs={[ETRADE_LOGO]} selected={broker === 'ETRADE'} onClick={() => { setBroker('ETRADE'); setStep(2); }} wide />
                  <LogoTile label="Tradier" srcs={[TRADIER_LOGO]} selected={broker === 'TRADIER'} onClick={() => { setBroker('TRADIER'); setStep(2); }} wide />
                  <LogoTile label="Coinbase" srcs={[COINBASE_LOGO]} selected={broker === 'COINBASE'} onClick={() => { setBroker('COINBASE'); setStep(2); }} wide />
                </div>

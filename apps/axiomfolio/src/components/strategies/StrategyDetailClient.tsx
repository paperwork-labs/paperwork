...
export default function StrategyDetailClient() {
  ...
  const strategyId = strategyIdFromParams(routeParams?.strategyId);
  ...
  if (!strategy) {
    return (
      <Page>
        ...
        <Button className="mt-4" onClick={() => router.push("/strategies")}>
          Back to Strategies
        </Button>
      </Page>
    );
  ...
  const entryRules = strategyParams.entry_rules as ConditionGroupData | undefined;
  const exitRules = strategyParams.exit_rules as ConditionGroupData | undefined;

  return (
    <ChartContext.Provider value={setChartSymbol}>
    <Page>
      <div className="mb-4">
        <Button size="sm" variant="ghost" className="gap-1" onClick={() => router.push("/strategies")}>
                ...
                {signals.map((sig, i) => (
                  <Card key={i} className="gap-0 border border-border shadow-none ring-0">
                    <CardContent className="py-3">
                      ...
                      {sig.symbol && (
                        <Link
                          href={`/market/tracked?symbols=${encodeURIComponent(sig.symbol!)}`}
                  ...
                  <CardContent className="flex flex-wrap items-center justify-between gap-3 py-3">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        router.push(`/market/tracked?${search.toString()}`);
                      }}
                    >

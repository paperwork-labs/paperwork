...
import { useActiveTab } from '@paperwork-labs/ui';
...
const SectionFooterLink: React.FC<{ href: string; label: string }> = ({ href, label }) => (
  <Link
    href={href}
      ...
      <div className="flex h-7 w-full overflow-hidden rounded-md">
        {entries.map(([stage, count]) => {
          ...
          return (
            <Link
              key={stage}
              href={`/market/tracked?filter_stage=${encodeURIComponent(stage)}`}
      ...
      {brief.stage_transitions?.length > 0 && (
        <Card className={cn('gap-0 py-4', CARD_RING)}>
          <CardContent>
            ...
            {transitionSymbols && (
              <SectionFooterLink
                href={`/market/tracked?symbols=${encodeURIComponent(transitionSymbols)}`}
      ...
      {brief.scan_changes?.length > 0 && (
        <Card className={cn('gap-0 py-4', CARD_RING)}>
          <CardContent>
            ...
            <SectionFooterLink
              href="/market/tracked?mode=scan&action_labels=BUY"
      ...
      {brief.exit_alerts?.length > 0 && (
        <Card className={cn('gap-0 py-4', CARD_RING)}>
          <CardContent>
            ...
            <SectionFooterLink
              href="/market/tracked?action_labels=REDUCE,AVOID"
          ...
          <div className="flex flex-col gap-1.5">
            {brief.sector_analysis.map((s: any) => {
              ...
              return (
                <Link
                  key={s.sector}
                  href={`/market/tracked?sectors=${encodeURIComponent(s.sector)}`}
    ...
    <Card className={cn('gap-0 py-4', CARD_RING)}>
      <CardContent>
        ...
        {footerTo && footerLabel && (
          <SectionFooterLink href={footerTo} label={footerLabel} />
        )}
      </CardContent>
    </Card>
  );

...
import { connectHubApi, type ConnectionBrokerOption } from "@/services/api";
...
function GridSkeleton() {
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3" aria-label="Loading brokers" aria-busy>
      {Array.from({ length: 6 }).map((_, i) => (
        <ChartGlassCard key={i} level="resting" padding="md" as="div" className="h-[150px]">
...
function ConnectAccountsContent() {
  ...
  const handleImport = React.useCallback(
    (broker: ConnectionBrokerOption) => {
      router.push(`/portfolio/import?broker=${broker.slug}`);
    },
    [router],
  );
  ...
  const filtered = React.useMemo(
    () => allBrokers.filter((b) => passesCategory(b, categoryFilter) && passesMethod(b, methodFilter)),
    [allBrokers, categoryFilter, methodFilter],
  );

  const connectedCount = allBrokers.reduce((acc, b) => acc + (b.user_state.connected ? 1 : 0), 0);
  ...
  return (
    <Page>
      <PageHeader
        ...
        rightContent={
          <Button type="button" variant="outline" size="sm" asChild>
            <Link href="/accounts/manage">Manage connections</Link>
          </Button>
        }
      />
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        ...
        {connectedCount > 0 ? (
          <span className="ml-auto text-xs text-muted-foreground">{connectedCount} connected</span>
        ) : null}
      </div>

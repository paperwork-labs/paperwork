...
const AccountRiskCard: React.FC<{ account: ManagedAccount }> = ({ account }) => {
  ...
  const [draft, setDraft] = React.useState<Partial<Record<keyof RiskProfileLimits, string>>>({});
  const [fieldError, setFieldError] = React.useState<Partial<Record<keyof RiskProfileLimits, string>>>(
    {},
  );
    ...
    return (
      <Card>
        <CardContent className="pt-6">
          <p className="text-sm text-destructive">Could not load risk profile for {account.account_number}.</p>
    ...
    <Card>
      <CardContent className="space-y-4 pt-6">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-muted-foreground">Effective = tighter of firm cap and your cap</span>
        </div>
              ...
              {FIELD_ORDER.map((field) => {
                ...
                return (
                  <tr key={field} className="border-b border-border/60 align-top">
                    <td className="py-3 pr-3 font-medium">{FIELD_LABELS[field]}</td>
                    <td className="py-3 pr-3 text-muted-foreground">{formatPct(query.data?.firm[field])}</td>
                    ...
                    <td className="py-3 pr-3 font-medium">{formatPct(query.data?.effective[field])}</td>
                  </tr>
                );
              })}
            </tbody>
...
export default function SettingsAccountRiskClient() {
    ...
    <div className="w-full">
      <div className="mx-auto w-full max-w-[960px] space-y-4">
        ...
        {accountsQuery.isLoading ? (
          <Card>
            <CardContent className="pt-6 text-sm text-muted-foreground">Loading accounts…</CardContent>
          </Card>
        ) : accountsQuery.isError ? (
          <Card>
            <CardContent className="pt-6">
              <p className="text-sm text-destructive">Could not load your accounts.</p>
              <Button type="button" variant="outline" size="sm" className="mt-2" onClick={() => accountsQuery.refetch()}>
                Retry
              </Button>
            </CardContent>
          </Card>
          ...
          accountsQuery.data.map((account) => <AccountRiskCard key={account.id} account={account} />)
        )}
      </div>
    </div>
  );

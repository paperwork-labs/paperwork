...
| P3.3 Articles of Organization PDF | Founder 1 | @react-pdf/renderer, state-specific templates | P3.2 | **DONE** (PR #63 - all 10 states: CA, TX, FL, DE, WY, NY, NV, IL, GA, WA) |
...
| P3.4d Playwright automation scripts (top 10) | Founder 1 | Headless browser automation for portal filing. Tier 2 handler. One script per state. RA/agent filing paths where available. **Files**: `packages/filing-engine/src/handlers/portal/{state_code}.ts`, shared helpers in `packages/filing-engine/src/handlers/portal/shared/`. **AC**: (1) All 10 states file successfully in staging. (2) Multiple selector strategies per field (CSS > XPath > text > ARIA). (3) Screenshot taken at every form page + confirmation. (4) Payment handled via Stripe Issuing virtual card. (5) RA login used where available. (6) Graceful failure -> manual fallback queue. | P3.4b, P3.4e | PARTIAL (PR #63 - handlers + shared helpers done, need live portal testing) |
...
| P3.7 LaunchFree dashboard | Founder 1 | User's LLC status, compliance checklist, RA status, next steps, filing submission status | P3.4, P3.4f | **DONE** (PR #63 - dashboard with formation list, detail, status timeline) |
| P3.8 Legal pages | Founder 1 | Privacy policy, ToS for LaunchFree (adapt from FileFree). UPL disclaimer on all formation pages. | P3.1 | **DONE** (PR #63 - privacy, terms, disclaimer with UPL compliance) |

...
Size:    $500k account: 1 ct @ $26.50 = $2,650 = 0.53% premium ✓
...
5. Each card's `score_breakdown` is collapsible, fully transparent ("HARD filters passed: liquidity ✓ earnings ✓ TD sell ✓; SOFT score components: RS=15/20, slope=10/10...").
...
4. Trade Card (G10) for an open position with no system data renders an explicit "⚠️ This symbol is outside our tracked universe — auto-backfill scheduled" badge instead of silently rendering empty fields.
...
5. **Trade Card (G10) renders external alignment row**: `System: BUY ✓  ·  StocksToBuyNow.ai: BUY ✓  ·  Zacks: HOLD`
6. **Divergence panel**: when system verdict and external consensus disagree (e.g., system AVOID + 2+ providers BUY), card renders with prominent `⚠️ Methodology disagreement` badge linking to a "why" expansion: which signals diverged, and what each one's track record is. **No auto-trade on divergence.** Symbol moves into `manual_review` queue.
...
3. **Trade Card (G10) for an open position renders runner-state badge prominently**:
   ...
   - 🎟️ Runner — "House money. Earnings risk acceptable. Stop is $0."
   - 🏔️ Pyramid — added to winner; treat as upgraded conviction, tighter stops
...
> "stockstobuynow.ai made me a lot of money which i didnt realize last years but was also making me loose money — so we gotta be careful.. I have not realized loses and thats why building AxiomFolio"
...
> "s&p is slow for me say active trader + i do say i buy indivuidal stocks and hold too - system recommended"
...
- Synced 1,666 trades ✓ + 29 cash transactions ✓ + 45 transfers ✓
- Synced **0 positions, 0 tax lots, 0 account balances** ✗
...
5. Tracks progress per year being imported (e.g., "2024 ✓, 2023 ✓, 2022 in progress, 2021 pending")

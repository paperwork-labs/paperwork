...
Bring the write-path (live and paper order placement) to parity with the sync-path (already multi-broker). Today `backend/services/execution/broker_router.py` only registers `ibkr` and `paper`; the sync side already ships IBKR, TastyTrade, Schwab, E*TRADE, Tradier, and Coinbase. This wave closes the gap.
...
- Executor protocol: [`backend/services/execution/broker_base.py`](../../backend/services/execution/broker_base.py) lines 91-111 (`BrokerExecutor` Protocol: `preview_order`, `place_order`, `cancel_order`, `get_order_status`, `is_paper_trading`)
- Router: [`backend/services/execution/broker_router.py`](../../backend/services/execution/broker_router.py) lines 44-52 (`create_default_router`)
- Reference live executor: [`backend/services/execution/ibkr_executor.py`](../../backend/services/execution/ibkr_executor.py)
- Reference paper executor: [`backend/services/execution/paper_executor.py`](../../backend/services/execution/paper_executor.py)
- Order manager (caller): [`backend/services/execution/order_manager.py`](../../backend/services/execution/order_manager.py)
- Risk gate (DANGER ZONE): [`backend/services/execution/risk_gate.py`](../../backend/services/execution/risk_gate.py) `_check_stage_regime_sizing` lines 181-239 assumes Weinstein stages
- Order enum: [`backend/models/order.py`](../../backend/models/order.py) lines 54-57 — only has `IBKR, TASTYTRADE, SCHWAB`. Live write path needs `ETRADE, TRADIER, COINBASE` added.
- Sync-side broker list: [`backend/services/portfolio/broker_sync_service.py`](../../backend/services/portfolio/broker_sync_service.py) lines 116-154 already knows all 7. `BrokerType` in `backend/models/broker_account.py` already has all values.
- OAuth token store: [`backend/models/broker_oauth_connection.py`](../../backend/models/broker_oauth_connection.py) `OAuthBrokerType` already includes `ETRADE, SCHWAB, TASTYTRADE, TRADIER, COINBASE`
- Broker REST clients (already built for sync, extend for write): [`backend/services/bronze/tradier/client.py`](../../backend/services/bronze/tradier/client.py), [`backend/services/bronze/etrade/client.py`](../../backend/services/bronze/etrade/client.py), [`backend/services/clients/schwab_client.py`](../../backend/services/clients/schwab_client.py), [`backend/services/clients/tastytrade_client.py`](../../backend/services/clients/tastytrade_client.py), [`backend/services/clients/coinbase_client.py`](../../backend/services/clients/coinbase_client.py) (if missing, Agent creates it)
...
Every live executor's `place_order`/`cancel_order` **must** call `ensure_valid_token()` before the first API call. Today, OAuth token refresh only runs in the sync path ([`backend/services/oauth/refresh.py`](../../backend/services/oauth/refresh.py)). If a trade is submitted with an expired token, the broker rejects it and we lose order-ID capture.

Shared helper to add in F0: `backend/services/execution/oauth_executor_mixin.py` — a mixin or small helper that wraps `OAuthBrokerAdapter.ensure_valid_token(connection)` and raises a typed error if refresh fails. All F1-F4 executors use it.
...
1. Extend `BrokerType` enum in [`backend/models/order.py`](../../backend/models/order.py) to include `ETRADE, TRADIER, TRADIER_SANDBOX, COINBASE`. (The column is `String(20)` so no Alembic needed for the enum itself — but add a migration `0075_broker_type_check.py` that adds a `CHECK (broker_type IN (...))` constraint to enforce valid values at the DB level.)
2. Add `CRYPTO_MAX_POSITION_PCT: float = 0.05` to [`backend/config.py`](../../backend/config.py) Settings.
3. Modify [`backend/services/execution/risk_gate.py`](../../backend/services/execution/risk_gate.py) per the crypto design above. Add unit test `backend/tests/execution/test_risk_gate_crypto.py` covering: BTC-USD passes a 3%-of-equity order; BTC-USD rejects a 10%-of-equity order; AAPL still routes through `_check_stage_regime_sizing`.
4. Create `backend/services/execution/oauth_executor_mixin.py` exposing `async def ensure_broker_token(connection: BrokerOAuthConnection) -> None`.
5. Create [`backend/services/execution/coinbase_paper_executor.py`](../../backend/services/execution/coinbase_paper_executor.py) — mirrors [`backend/services/execution/paper_executor.py`](../../backend/services/execution/paper_executor.py) but: (a) sets `broker_name = "coinbase-paper"`; (b) tags every `Order.notes` with `"paper"`; (c) rejects symbols that do not match `_is_crypto_symbol`.
6. Register `coinbase` (paper) in `create_default_router` in [`backend/services/execution/broker_router.py`](../../backend/services/execution/broker_router.py).
...
1. Create [`backend/services/execution/tradier_executor.py`](../../backend/services/execution/tradier_executor.py). Model after `ibkr_executor.py`. Implement `preview_order` via Tradier's `/v1/accounts/{id}/orders?preview=true`, `place_order` via POST `/v1/accounts/{id}/orders`, `cancel_order` via DELETE, `get_order_status` via GET. Use `TradierClient` from [`backend/services/bronze/tradier/client.py`](../../backend/services/bronze/tradier/client.py); extend that client if order endpoints are not yet exposed.
...
Complications: OAuth 1.0a (not 2.0); HMAC-SHA1 request signing. The signing is already implemented for sync in [`backend/services/bronze/etrade/client.py`](../../backend/services/bronze/etrade/client.py) lines 65-71.
...
Replace the `not_implemented` stub at [`backend/services/clients/schwab_client.py`](../../backend/services/clients/schwab_client.py) lines 506-511. Schwab Trader API requires an `account_hash` (not the plain `account_id`) — the sync service already resolves this; reuse the same resolver. Status requires polling since Schwab does not push; the executor's `get_order_status` polls the Schwab order endpoint.
...
2. Backend: ensure [`backend/services/execution/coinbase_paper_executor.py`](../../backend/services/execution/coinbase_paper_executor.py) cannot be "upgraded" to live by a config typo — add a module-level `assert os.getenv("COINBASE_ALLOW_LIVE") != "true"` guard with a clear error message if set.
...
| Risk | Mitigation |
|---|---|
| OAuth token race between sync and write paths | F0 mixin serializes `ensure_valid_token` under a per-connection Redis lock (already available via `backend.services.cache.redis_client`, see R40) |

...
> Wires the empty IV pipeline to free providers first (Yahoo + IBKR Gateway) and defers the paid-provider decision until after 30 days of coverage measurement. Resolves R-IV01 (silent-zero in Options tab) and the explicit NOOP in `backend/tasks/market/iv.py`.
...
- Model: [`backend/models/historical_iv.py`](../../backend/models/historical_iv.py) — columns `iv_30d`, `iv_60d`, `iv_rank_252`, `iv_high_252`, `iv_low_252`, `hv_20d`, `hv_60d`, `iv_hv_spread`.
- Contract-level model: [`backend/models/market/options_chain_snapshot.py`](../../backend/models/market/options_chain_snapshot.py) lines 36-38 — `implied_vol`, `iv_pctile_1y`, `iv_rank_1y`.
- Ranking helper: [`backend/services/gold/options_chain_surface.py`](../../backend/services/gold/options_chain_surface.py) lines 120-139 — `_iv_percentile_rank`.
- Rank computation task: [`backend/tasks/market/iv.py`](../../backend/tasks/market/iv.py) `compute_rank` lines 39-111 — reads `HistoricalIV.iv_30d`, writes `iv_rank_252`, `iv_high_252`, `iv_low_252`, `iv_hv_spread`.
- IBKR option-chain API: [`backend/services/clients/ibkr_client.py`](../../backend/services/clients/ibkr_client.py) `get_option_chain` lines 433-501.
- Yahoo options helper: [`backend/services/market/yfinance_options_chain.py`](../../backend/services/market/yfinance_options_chain.py) (has `_parse_iv` at lines 40-48 that normalizes Yahoo's IV-as-fraction-or-percent into a `Decimal`).

**The actual gap:**

- [`backend/tasks/market/iv.py`](../../backend/tasks/market/iv.py) lines 22-32 — `snapshot_iv_from_gateway` is an explicit NOOP, so `HistoricalIV` is never populated and `compute_rank` operates on an empty series forever.
...
[`backend/services/market/historical_iv_service.py`](../../backend/services/market/historical_iv_service.py) — new module:
...
- Yahoo ATM lookup: fetch nearest-expiry option chain (≥7 DTE, ≤45 DTE preferred; skip if outside), pick the call+put pair with strike closest to spot, take the mid of their IVs. Reuse `_parse_iv` from [`backend/services/market/yfinance_options_chain.py`](../../backend/services/market/yfinance_options_chain.py) lines 40-48.
- IBKR ATM: use `ibkr_client.get_option_chain(symbol)` filtered to front-month; primary path when gateway is up.

### 2. Replace the NOOP

Rewrite [`backend/tasks/market/iv.py`](../../backend/tasks/market/iv.py) `sync_gateway`:
...
[`backend/tasks/job_catalog.py`](../../backend/tasks/job_catalog.py) — add `snapshot_iv_from_gateway` to the catalog:
...
[`backend/services/market/scan_engine.py`](../../backend/services/market/scan_engine.py) — add `iv_rank_252` as a selectable filter dimension. Reference implementation pattern: how `rs_mansfield` is currently plumbed (see R29 — it correctly skips symbols with `None` instead of coercing to 0).
...
[`backend/services/monitoring/admin_health_service.py`](../../backend/services/monitoring/admin_health_service.py) — add an `iv_coverage` dimension:

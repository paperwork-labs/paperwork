...
Living notes from API / market-data audits. **Use one consistent figure for route-module scope:** there are **40** Python modules under `backend/api/routes/` (including packages such as `market/` and `portfolio/`). Do not mix that count with older informal totals (e.g. “85+” or “93”) that referred to endpoints or lines—those are not comparable.
...
Source: `backend/services/market/rate_limiter.py`.

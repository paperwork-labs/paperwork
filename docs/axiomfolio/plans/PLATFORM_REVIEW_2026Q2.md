...
**Axis 1 — Stripe MRR.** Paid subscribers on Free / Pro / Pro+ / Quant Desk / Enterprise. Pricing locked per [`app/services/billing/tier_catalog.py`](../../app/services/billing/tier_catalog.py): $0 / $29 / $79 / $299 / custom. Annual SKUs at 2-months-free: $0 / $290 / $790 / $2,990 / custom.
...
Implementation: bump `SCHEMA_VERSION` in [`app/api/routes/portfolio/tax_export.py`](../../app/api/routes/portfolio/tax_export.py); update Pydantic models in [`app/services/tax/filefree_exporter.py`](../../app/services/tax/filefree_exporter.py); add unit tests round-tripping a 2025-style trade log through the new fields; Brain webhook endpoint contract unchanged (HMAC-signed, idempotent).
...
├─────────────────────────────────────────────────────────────────────┤
│  SILVER app/services/silver/  (grandfathered today: market/,    │
│                                      portfolio/, tax/)               │
...
├─────────────────────────────────────────────────────────────────────┤
│  BRONZE app/services/bronze/<broker>/                            │
...
- New modules land in `app/services/<layer>/` from day one.
...
- **Indicator engine** (`app/services/market/indicator_engine.py`): full Stage Analysis (SMA150-anchored 10-sub-stage), RSI, ATR, ADX, MACD, TD Sequential, RS Mansfield. All monetary math in `Decimal`.
- **Execution pipeline** (`app/services/execution/`): `OrderManager` → `RiskGate` → `PreTradeValidator` → `BrokerRouter`. Paper / shadow / live modes. `ExitCascade` for stops + trails. `CircuitBreaker` for drawdown gates.
- **Regime engine** (`app/services/market/regime_engine.py`): R1–R5 scoring from 6 inputs; gates long/short/flat access.
- **Backtest** (`app/services/backtest/`): `BacktestEngine`, bar-replay, `SimulatedOrderManager` (parity with live). Walk-forward and Monte Carlo are Phase 6 (not shipped).
- **Tax lot selector** (`app/services/tax/`): partial G16; FIFO / LIFO / HIFO / specific-lot; wash-sale guard present but single-broker today.
...
- **Picks pipeline** (`app/services/picks/`): inbound webhook, polymorphic LLM parser, validator queue, tier-gated publish.
- **FileFree export** (`app/services/tax/filefree_exporter.py`): schema 1.0.0 live via Brain webhook.
...
Tier catalog per [`app/services/billing/tier_catalog.py`](../../app/services/billing/tier_catalog.py):
...
Current taglines per [`app/services/billing/tier_catalog.py`](../../app/services/billing/tier_catalog.py):
...
These are two-line copy changes in [`tier_catalog.py`](../../app/services/billing/tier_catalog.py) with Vitest snapshot updates. Ship as part of Wave 7 (launch marketing).

### Feature-to-tier remap

Current [`app/services/billing/feature_catalog.py`](../../app/services/billing/feature_catalog.py) places `strategy.hnw_tax_deferral` at PRO_PLUS. Per Chapter 1.7 G20 tier-gating resolution, this needs to split. New proposed mapping for features added by this review:
...
- **Schwab**: OAuth + Trader API client methods exist in `app/services/clients/schwab_client.py`. Not routed through `OrderManager` for live orders yet.
...
- `BacktestEngine` in `app/services/backtest/`, bar-replay via `SimulatedOrderManager` (parity with live `OrderManager`).
...
- Danger-zone touches: `app/services/execution/circuit_breaker.py`, `app/services/execution/risk_gate.py`. Get founder approval per `protected-regions.mdc` before modifying.
...
- Tagline rewrites ([`tier_catalog.py`](../../app/services/billing/tier_catalog.py)).
...
- Every file under `app/services/` is either inside `bronze/ | silver/ | gold/` or has an explicit `# medallion: grandfathered; move in issue #NNN` docstring tag.
...
- `app/services/execution/risk_gate.py` (W1, W13, W3.6, W3.7)
- `app/services/execution/order_manager.py` (W3.6, W5)
- `app/services/execution/exit_cascade.py` (W3.5)
- `app/services/risk/circuit_breaker.py` (W1, W13)
- `app/services/market/indicator_engine.py` (no wave touches; confirmed)
- `app/services/market/stage_classifier.py` (no wave touches; confirmed)
- `app/services/market/regime_engine.py` (no wave touches; confirmed)
- `app/api/routes/auth.py` (W15 only; no earlier waves)
- `backend/config.py`, Alembic versions, `app/tasks/job_catalog.py` (W1, W3, W8, W13)

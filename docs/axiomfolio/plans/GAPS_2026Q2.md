...
**Root cause hypothesis:** [app/tasks/market/coverage.py](../../app/tasks/market/coverage.py) `_run_scan_overlay` calls `get_current_regime(session)` to make the gating decision, but never writes `regime.regime_state` back onto the `MarketSnapshot.regime_state` column.
...
The `candidates` table is empty. The generator class exists at [app/services/picks/generators/stage2a_rs_strong.py](../../app/services/picks/generators/stage2a_rs_strong.py) but no Celery Beat entry triggers it. The "Buy Candidates" UI is currently reading from `market_snapshot.scan_tier='Breakout Elite' AND action_label='BUY'` (the scan-overlay path), not from the dedicated `Candidate` model that Phase 0.5 designed.

**Acceptance criteria:**
1. New Beat entry in [app/tasks/job_catalog.py](../../app/tasks/job_catalog.py) running daily after `scan_overlay` (Step 6 of `daily_bootstrap`).
...
2. Doc note in [app/services/market/stage_classifier.py](../../app/services/market/stage_classifier.py) explaining the lookback window definition.
...
```
app/services/picks/trade_card_composer.py
  inputs:
...
# app/models/external_signals.py
...
```python
# app/models/portfolio_position.py
...
```python
# app/services/portfolio/position_health.py
...
```python
# app/services/signals/peak_signal.py
...
1. New service `app/services/signals/peak_signal.py` with `PeakSignalEngine.evaluate(symbol, position) -> PeakSignalResult`
...
7. **Tier-gating**: peak signals visible to all paying users (Pro+); the **specific recommended scale percentage and EV math** is Pro+ (composes with G16 tax overlay)
...
```python
# app/services/exits/tax_aware_exit.py
...
```python
# app/services/risk/portfolio_leverage.py
...
# app/services/portfolio/sleeve_classifier.py
...
8. **Per-tier visibility**: free users see sleeve tags but no override; Pro+ can override per position and configure custom sleeves.
...
```python
# app/services/picks/generators/conviction_secular.py
...
- **Fundamentals layer**: revenue / earnings / FCF / ROIC time series per symbol. Provider: FMP fundamentals endpoint or Polygon. New service `app/services/fundamentals/`.
...
7. **Tier-gating**: Conviction Picks are tier-restricted. Free = top 1 pick with 7-day delay. Pro = full ranked list real-time. Pro+ = + per-pick "why this stock" LLM brief generated by AgentBrain (D83 native chat).
...
```python
# app/services/tax/deferral_alternatives.py
...
6. **Tier-gating**: Free = no surface. Pro = SBLOC + options awareness. Pro+ = full alternatives engine + concierge "request advisor intro" workflow.
...
```python
# app/services/replay/founder_replay.py
...
```python
# app/services/portfolio/ibkr/pipeline.py
...
2. Per-step `StepResult` returned by every `sync_*` function in `app/services/portfolio/ibkr/sync_*.py`
...
```python
# app/api/routes/portfolio/historical_import.py
...
```python
# app/services/portfolio/onboarding/historical_wizard.py
...
```python
# app/services/tax/account_type_router.py
...
1. New `app/services/tax/account_type_router.py` with strategy classes for all 6 enum values
...
```python
# app/services/portfolio/onboarding/account_discovery.py
...
```python
# app/models/account_risk_profile.py
...
# app/services/execution/risk_gate.py reads account_risk_profile per account_id
...
## G28 — Deploy Health Telemetry + Validator-Specific PARTIAL Messaging

- **Severity**: P0 (surfaced live 2026-04-20; caused 8+ hours of silent prod drift; risk repeats every deploy until fixed)
- **Phase**: Phase 0 (correctness guardrail; ships alongside G22 follow-ups)
- **Effort**: S (Render API poller + `/admin/health` row + `gh` pre-merge check + validator message improvement) — 1–2 days
- **Suggested owner**: Background Agent

**Symptom (discovered 2026-04-20):**

Between 2026-04-20 19:48 UTC and 2026-04-21 03:10 UTC, **every Render deploy of both `axiomfolio-api` and `axiomfolio-worker` failed at queue time** (status `build_failed` with `finishedAt - createdAt ≈ 1–2s`, meaning the build never started — Render rejected it, likely a billing / build-minutes / account-state issue). CI was green on every merged PR. `main` was green. PR pages showed `MERGED`. No alert fired anywhere in AxiomFolio's own telemetry.

Commits stuck out of prod during that window (partial list):

- `ccedcf69` — PR #383 — G22 sync completeness validation (migration 0053)
- `b0b29693` — PR #384 — docs cleanup
- `618f6db6` — PR #382 — G1-G27 gaps + D116-D119 decisions
- `a2d0e33b` — PR #353 — dependabot frontend bump (security patches)
- `6a96cc27` — PR #376 — Trade Decision Explainer (migration 0051)
- `9b742b78` — OAuth foundation + E*TRADE sandbox adapter
- `4e139e5f` — Multi-tenant rate limits + GDPR rights

Meanwhile a manually-triggered IBKR sync (`account_syncs.id=5`) started at 03:47 UTC running on pre-G22 code — which cannot set `PARTIAL`, cannot populate `warnings`/`missing_sections` columns (they don't exist in the prod schema yet), and will silently mark `SUCCESS` even if the data came back empty. **The G22 feature we just shipped to fix this exact bug was not live when the bug re-occurred.**

**Principle violated:** `production-verification.mdc` already states "CI green ≠ shipped; verify Render deploy status." The rule exists; it was not operationally enforceable because no alarm fires when a deploy fails to start, and humans (including agents) default to treating "PR merged + CI green" as equivalent to "in prod."

**Architecture (three interlocking guardrails):**

**Guardrail 1 — Deploy Health Telemetry (backend):**

```python
# app/tasks/ops/deploy_health.py (new, Beat every 5 minutes)

@celery_app.task(name="ops.deploy_health_poll", time_limit=60, soft_time_limit=45)
def poll_deploy_health():
    """Poll Render API for latest deploys on all prod services;
    emit P0 alert if ≥2 consecutive build_failed OR stale live > 60min
    while commits-ahead-of-prod ≥ 1.

    Writes a `DeployHealthSnapshot` row per service per poll and a
    `Alert(kind='prod_deploys_stuck')` row when degraded.
    """
    services = settings.RENDER_PROD_SERVICES  # ["axiomfolio-api", "axiomfolio-worker"]
    for svc_name in services:
        deploys = render_api.list_deploys(svc_name, limit=5)
        latest = deploys[0]
        last_live = next((d for d in deploys if d.status == "live"), None)
        consecutive_failed = sum(
            1 for d in deploys if d.status == "build_failed" and
            (d.finished_at - d.created_at).total_seconds() < 10
        )
        commits_ahead = github_api.compare(last_live.commit.id, "main").ahead_by
        degraded = consecutive_failed >= 2 or (
            last_live.created_at < utcnow() - timedelta(minutes=60)
            and commits_ahead >= 1
        )
        persist_snapshot(svc_name, latest, last_live, commits_ahead, degraded)
        if degraded:
            emit_alert(
                kind="prod_deploys_stuck",
                severity="P0",
                service=svc_name,
                last_live_sha=last_live.commit.id,
                consecutive_failed=consecutive_failed,
                commits_ahead=commits_ahead,
                message=f"{svc_name}: last {consecutive_failed} deploys failed; "
                        f"prod is {commits_ahead} commits behind main "
                        f"(last live: {last_live.commit.id[:8]})",
            )
```

Surface on `/admin/health` as a dimension row "Prod deploys":

- green: last deploy live, 0 commits ahead
- amber: last deploy live but 1–3 commits ahead (pending auto-deploy)
- red: ≥2 consecutive `build_failed` OR ≥4 commits ahead for >60 min — shows stuck service name + last-live SHA + link to Render dashboard

**Guardrail 2 — Pre-merge deploy assertion (CI):**

```yaml
# .github/workflows/prod-deploy-healthcheck.yml (new, required check)
# Runs on pull_request; fails the PR (or posts a sticky warning comment) if
# the last deploy on main for axiomfolio-api OR axiomfolio-worker is not `live`.

jobs:
  check_prod_deploy:
    runs-on: ubuntu-latest
    steps:
      - name: Query Render deploys
        run: |
          api_state=$(curl -H "Authorization: Bearer $RENDER_API_KEY" \
            "https://api.render.com/v1/services/$API_SVC/deploys?limit=1" \
            | jq -r '.[0].status')
          worker_state=$(curl -H "Authorization: Bearer $RENDER_API_KEY" \
            "https://api.render.com/v1/services/$WORKER_SVC/deploys?limit=1" \
            | jq -r '.[0].status')
          if [[ "$api_state" != "live" || "$worker_state" != "live" ]]; then
            echo "::warning::Prod is not green. api=$api_state worker=$worker_state. Merging this PR will compound the backlog."
            # Post sticky comment on PR, do NOT auto-block merge (admin can override)
            gh pr comment "$PR_NUMBER" --body "[warn] Prod deploy health: api=$api_state worker=$worker_state. Consider waiting for prod to recover before merging."
          fi
```

**Guardrail 3 — Validator-specific PARTIAL messaging (UI + log):**

Currently G22's `PARTIAL` surface reads "missing required broker report sections ['OpenPositions']" — that's useful. But for every PARTIAL reason other than `missing_required`, the message falls back to a generic "pipeline reported degraded completeness; see sync history for details." That's what caused the 2026-04-20 agent-diagnosis error: the agent inferred *which* sections were missing instead of reading the exact list from a validator report. Fix:

```python
# app/services/portfolio/broker_sync_service.py (_build_partial_sync_message)
# Extend to ALWAYS include the exact list of missing section names + the exact
# pipeline_step_errored section names + the per-section row counts for sections
# that had ZERO rows (but were present) — so operators never have to infer.

def _build_partial_sync_message(completeness):
    parts = []
    missing = completeness.get("missing_required", [])
    if missing:
        parts.append(f"missing required sections: {missing}")
    errored = [w["section"] for w in completeness.get("warnings", [])
               if w.get("code") == "pipeline_step_errored"]
    if errored:
        parts.append(f"writer errored on: {errored}")
    zero_row = [sec for sec, count in completeness.get("section_row_counts", {}).items()
                if count == 0 and sec in completeness.get("received_sections", [])]
    if zero_row:
        parts.append(f"sections present but empty: {zero_row}")
    # ... always populate at least one, never fallback to generic
```

Matching UI change: `SettingsConnections.tsx` PARTIAL tooltip renders three distinct sub-sections (missing / errored / empty) instead of a single flat string; each sub-section links to the IBKR Flex-Query step that remediates it.

**Acceptance criteria:**

1. `deploy_health_poll` Beat job runs every 5 minutes; persists `DeployHealthSnapshot` + emits `Alert` when degraded
2. `/admin/health` shows "Prod deploys" dimension (green / amber / red) with service-level breakdown
3. `prod-deploy-healthcheck.yml` workflow posts a sticky PR comment when prod is not green (warning, not hard-block)
4. G22 PARTIAL message always lists exact section names (missing / errored / empty-but-present); never falls back to a generic string if any structured info is available
5. UI tooltip on `SettingsConnections.tsx` PARTIAL badge renders the three sub-lists as distinct bullet lists, each with remediation hint
6. **Regression test**: replay the 2026-04-20 scenario — set Render API mock to return 2 × `build_failed` deploys, assert `Alert` row created + `/admin/health` shows red; set `received_sections=['OpenPositions']` + `section_row_counts={'OpenPositions': 0}`, assert PARTIAL message mentions "empty: ['OpenPositions']" not a generic string
7. **Retroactive one-time action**: when this gap lands, add an `/admin/ops/redeploy-stuck-services` admin button that triggers a clear-and-re-push deploy for any service currently showing ≥2 consecutive failures

**Anti-goal:**

Never let this become a deploy-blocker that fires on legitimate Render hiccups (single transient failure). The two-consecutive-failures threshold is the minimum that rules out transient noise while still catching the 8-hour-silent-drift scenario. Do not hard-block merges on deploy health (admin might need to merge a hotfix into a degraded prod); a visible warning is enough.

**Multi-account application:** none (infra gap, not per-account). But the same mental model applies to G25's weekly broker-account re-discovery poller — if that poller ever stops running, `/admin/health` must go red the same way.

**Companion:** `production-verification.mdc` (now operationally enforceable via this gap); D120 + D121 in `KNOWLEDGE.md`; G22 follow-up.

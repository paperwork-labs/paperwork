...
This document classifies every `.py` file under `app/services/` into a medallion layer and records the target move path + risk + blockers. It's the **input contract for Phase 0.C** — the migration script `apis/axiomfolio/scripts/medallion_migrate.py` consumes this mapping.
...
Resolution in `apis/axiomfolio/scripts/medallion/generate_move_map.py`:
...
Full per-file move map lives at `medallion_move_map.yaml` (generated by `apis/axiomfolio/scripts/medallion/generate_move_map.py`). Summary:
...
Regenerate with:
```bash
python3 apis/axiomfolio/scripts/medallion/generate_move_map.py --stats   # summary
python3 apis/axiomfolio/scripts/medallion/generate_move_map.py > medallion_move_map.yaml
```
...
1. **Migration script** at `apis/axiomfolio/scripts/medallion_migrate.py`:

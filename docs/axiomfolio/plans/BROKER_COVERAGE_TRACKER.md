...
|------------|-------|-------------------|
| Accounts sync | LIVE | `app/services/portfolio/ibkr/flex_query_service.py`; FlexQuery nightly + on-demand |
...
| Order placement — equities | **LIVE** | `ib_insync` via `app/services/clients/ibkr_client.py` → `app/services/execution/broker_router.py` |
...
| Capability | State | Notes + code path |
|------------|-------|-------------------|
| Accounts sync | LIVE | OAuth via `app/services/clients/schwab_client.py`; `app/services/portfolio/schwab_sync_service.py` |
...
| Capability | State | Notes + code path |
|------------|-------|-------------------|
| Accounts sync | LIVE | TastyTrade SDK via `app/services/clients/tastytrade_client.py` |

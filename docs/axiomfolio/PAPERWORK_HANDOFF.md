...
1. **Internal AgentBrain** (`app/services/agent/brain.py`) — 55 tools for health remediation, interactive chat, and market analysis. Runs inside the AxiomFolio process. NOT exposed to Brain.

2. **Brain HTTP Tools** (`app/api/routes/brain_tools.py`) — 12 curated endpoints that Paperwork Brain calls via MCP. These are the tools listed below.
...
All scheduling is driven by Celery Beat from `app/tasks/job_catalog.py`. Render cron jobs have been retired.
...
- `app/api/routes/brain_tools.py` for endpoint implementation
- `app/services/brain/webhook_client.py` for webhook details (HMAC-SHA256 signing)
- `app/services/execution/approval_service.py` for approval logic

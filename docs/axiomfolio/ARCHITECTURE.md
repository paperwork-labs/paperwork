...
**CI enforcement (Phase 0.B):** `make medallion-check` (and the "Medallion import-layer gate" CI job) parses every `.py` under `app/services/`, reads its `medallion: <layer>` docstring tag, and asserts each `app.services.*` import respects the hierarchy. Violations fail CI. Waive a specific import with `# medallion: allow <reason>` on the import line.
...
Broker-specific **bronze adapter contracts** for packages under `app/services/bronze/` (entry points, counters, OAuth usage) are in [Data Pipelines](#data-pipelines) under the **Bronze layer** heading.
...
Brain calls AxiomFolio under `/api/v1/tools/*` (`app/api/routes/brain_tools.py`; manifest `docs/brain/axiomfolio_tools.yaml`). The contract includes read, trade, and ops helpers:
...
AxiomFolio notifies Brain with **HMAC-SHA256–signed** HTTP POSTs: JSON body signed with `BRAIN_WEBHOOK_SECRET`, sent as `X-Webhook-Signature: sha256=<hex>`. The request URL is `{BRAIN_WEBHOOK_URL}/api/v1/webhooks/axiomfolio` (see `app/services/brain/webhook_client.py`).
...
The **internal agent** (`AgentBrain` / **AxiomBrain** in `app/services/agent/brain.py`) is **not** the same surface as Brain HTTP tools. It is the in-app domain intelligence layer: health remediation (auto-ops), admin chat, and a large set of internal tools in `app/services/agent/tools.py` (DB, market insight, schedules, codebase read, backtests, etc.). Those tools are invoked as Python callables inside the API/worker, not exposed on `/api/v1/tools/*`.
...
Routers are mounted from `app/api/main.py`. Portfolio routes sit under `portfolio/`; market data is split across `market/*.py` and mounted at `/api/v1/market-data`.
...
Beat loads **`app/tasks/job_catalog.py`** (`CATALOG`) into `celery_app` `beat_schedule` (`app/tasks/celery_app.py`). Example entries (see catalog for the full list):

| Catalog `id` | Celery task | Default schedule | Purpose |
|------|-------------|------------------|---------|
| `admin_coverage_backfill` | `app.tasks.market.coverage.daily_bootstrap` | `0 1 * * *` UTC | Nightly chain: constituents, tracked cache, daily bars, `recompute_universe`, regime, scan overlay, snapshot history, exit/strategy eval, `health_check` (coverage cache), audit, digest |
| `ibkr-daily-flex-sync` | `app.tasks.account_sync.sync_all_ibkr_accounts` | `15 2 * * *` UTC | IBKR FlexQuery sync |
| `fundamentals_fill` | `app.tasks.market.fundamentals.fill_missing` | `15 3 * * *` UTC | Fill missing fundamentals on snapshots |
| `auto_ops_health_check` | `app.tasks.auto_ops_tasks.auto_remediate_health` | `*/15 * * * *` UTC | Auto-ops remediation loop |
...
### Bronze layer (`app/services/bronze/<broker>/`)
...
package under `app/services/bronze/`:
...
- **Credentials**: loaded from `BrokerOAuthConnection` filtered by `user_id`
  ...
  decrypted via `app.services.oauth.encryption.decrypt`. For OAuth 1.0a
...
Fan-out task: one module per broker under `app/tasks/portfolio/` (e.g.
`etrade_sync.sync_all_etrade_accounts`, `tradier_sync.sync_all_tradier_accounts`) — kept separate from the umbrella
`app.tasks.account_sync` namespace so per-broker fan-out modules stay
small. Each task declares explicit `time_limit` / `soft_time_limit`
matching `JobTemplate.timeout_s` in `app/tasks/job_catalog.py`
...
Query path: Redis cache → MV → raw table fallback. Managed by `MarketMVService` in `app/services/market/market_mv_service.py`.
...
- JWT helpers in `app/api/security.py`.
- All routes resolve current user via `app/api/dependencies.py`.
...
- **Source of truth**: All recurring jobs are defined in the job catalog (`app/tasks/job_catalog.py`). In production on Render, Beat is embedded in the worker process (`--beat` flag). Legacy Render cron jobs are suspended but the sync code (`render_sync_service.py`) remains for potential non-Beat deployments.
...
- **Agent**: Schedule inspection and ad-hoc dispatch use `list_schedules` and `run_task_now` in `app/services/agent/tools.py`.
...
- **Nameservers**: `janet.ns.cloudflare.com`, `noel.ns.cloudflare.com` (delegation configured at registrar **Spaceship**; DNS zone on Paperwork Labs work Cloudflare account — see `docs/runbooks/cloudflare-ownership.md`)
...
|-----------|------|---------|
| SignalEngine | `app/services/engine/signal_engine.py` | Consumes prices, evaluates strategies, emits signals |
| CircuitBreaker | `app/services/risk/circuit_breaker.py` | 3-tier daily loss limits (2%/3%/5%) |
| PreTradeValidator | `app/services/risk/pre_trade_validator.py` | Position sizing, concentration checks |
| OrderManager | `app/services/execution/order_manager.py` | Single order path, preview → submit → fill |
| BrainWebhookClient | `app/services/brain/webhook_client.py` | Notifies Brain of trades, alerts, approvals |
...
**Routing**: `app/tasks/celery_app.py` `task_routes` maps task names to queues. Heavy queue receives all multi-hour CPU-bound or memory-bound jobs so they don't block the fast queue (which handles dashboard warming, exit cascade evaluation, account sync, order processing).

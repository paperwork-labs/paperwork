...
```
app/services/portfolio/ibkr/sync_validator.py   (NEW — 250 lines, no DB, pure XML→CompletenessReport)
app/alembic/versions/0053_add_account_sync_completeness.py  (NEW)
...
```
app/models/broker_account.py            (+5 JSON columns on AccountSync)
app/services/portfolio/ibkr/pipeline.py (validate_completeness call + status branching)
app/services/portfolio/broker_sync_service.py (PARTIAL handling + _build_partial_sync_message helper)
app/tasks/portfolio/sync.py             (persist completeness payload + use shared helper)
app/api/routes/settings/account.py      (expose last_sync_completeness on /sync-status)
...
- New `IBKRHistoricalImporter` service in `app/services/portfolio/ibkr/historical_import.py`
...
- New `POST /api/v1/accounts/{id}/historical-import` route that accepts `{from_date, to_date}` and dispatches a Celery task `app.tasks.portfolio.historical_import.run_historical_import`.
...
**What**: The system currently treats every account the same. The founder's IRA and Joint Taxable need different exit logic (no wash-sale on IRA, no LTCG/STCG distinction on IRA, no tax-loss-harvesting in IRA). The `AccountType` enum already supports `IRA / ROTH_IRA / TAXABLE / HSA / TRUST / BUSINESS` (verified in `app/models/broker_account.py`); the gap is everything downstream that ignores it.

**Architecture**:

- New `app/services/strategy/account_strategy.py` module exposing `get_strategy_profile(account: BrokerAccount) -> AccountStrategyProfile`.
...
- **Wizard upgrade**: when the system detects a missing required section in the founder's Flex Query, render a "Fix your Flex Query" wizard with screenshot-by-screenshot instructions for adding that exact section. The instructions live in `IBKRFlexQueryClient.get_setup_instructions()` (already exists in `app/services/clients/ibkr_flexquery_client.py`); we just need to surface them contextually in the UI when G22 detects a gap.

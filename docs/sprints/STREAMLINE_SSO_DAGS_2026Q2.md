...
- shipped 2026-04-26: **T1 (Infra Health, interval)** — `BRAIN_OWNS_INFRA_HEALTH` + `brain_infra_health` cut **Infra Health Check** to Brain: **first first-party `IntervalTrigger`** (30m, not `CronTrigger`), n8n API parity with `infra-health-check.json`, fingerprint dedup + optional `INFRA_HEALTH_REMINDER_HOURS` re-alert, `n8n_shadow_infra_health` suppressed. Code: `apis/brain/app/schedulers/infra_health.py`. Runbook: [docs/infra/BRAIN_SCHEDULER.md](../infra/BRAIN_SCHEDULER.md).

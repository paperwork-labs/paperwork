...
- shipped 2026-04-26: **T3.2 (Trinkets)** — `<ClerkProvider>`, sign-in/up catch-all routes, `clerkMiddleware` (public utility app; no legacy session or Basic Auth to compose). Trinkets Clerk appearance: indigo `#6366F1` + sky cyan `#38BDF8`. Runbook: [docs/infra/CLERK_TRINKETS.md](../infra/CLERK_TRINKETS.md).
...
- **T3 — Clerk SSO (cross-product)** `[~]`
  ...
  - `[x]` LaunchFree + FileFree + **Trinkets** wired with `<ClerkProvider>` + sign-in/up + `clerkMiddleware` (LaunchFree/FileFree: legacy + Basic composition — PRs #155, #156; Trinkets: public shell — [CLERK_TRINKETS.md](../infra/CLERK_TRINKETS.md)).

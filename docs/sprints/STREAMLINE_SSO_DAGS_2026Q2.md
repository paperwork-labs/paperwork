...
- in progress: **T1.4** — `BRAIN_OWNS_CREDENTIAL_EXPIRY` + `brain_credential_expiry` cut the **Credential Expiry Check** n8n cron to Brain (same vault + `#alerts` Slack as `credential-expiry-check.json`); `n8n_shadow_credential_expiry` suppressed. Pattern: same as T1.2 / T1.3. Code: `apis/brain/app/schedulers/credential_expiry.py`. Runbook: [docs/infra/BRAIN_SCHEDULER.md](../infra/BRAIN_SCHEDULER.md). PR: see tracker.

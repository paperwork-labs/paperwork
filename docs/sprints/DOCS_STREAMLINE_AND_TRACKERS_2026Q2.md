...
- **Vercel — Studio prod redeploy of `f0255542`**: PR #142's merge commit on `main` was never picked up by Vercel (webhook miss — zero deployment record for that SHA, project `live: false`, last prod deploy was PR #141's `752c573`). Manual `vercel redeploy` hit the team's free-tier daily 100-deploy ceiling (`api-deployments-free-per-day`), so the fix has to ride PR #143's merge or wait 24h. Stale prerender of `/admin/secrets` reads `process.env.DATABASE_URL` at build time and hard-codes "DATABASE_URL not set" — this resolves once #142's `force-dynamic` change is in production.

...
- shipped 2026-04-25: F-1 + F-2 + F-4 — consolidated `apis/axiomfolio/render.yaml` into root `render.yaml` so all 8 Paperwork-managed Render services (FileFree API, Brain API, AxiomFolio API + 2 workers + frontend, AxiomFolio Redis, AxiomFolio DB) live in a single Blueprint. Old subtree blueprint neutered with retirement comment + dashboard-association steps. Dropped the `launchfree-api` stub (F-2). (PR #143)
...
- **F-1 (operator, single click)**: Render dashboard → Blueprints → New Blueprint → repo `paperwork-labs/paperwork`, branch `main`, blueprint path `render.yaml`, choose "Associate existing services". Walkthrough: [docs/infra/RENDER_REPOINT.md § Path B](../infra/RENDER_REPOINT.md). All 8 services land in one Blueprint after that.
- **Vercel — Studio prod redeploy of `f0255542`**: PR #142's merge commit on `main` was never picked up by Vercel (webhook miss — zero deployment record for that SHA, project `live: false`, last prod deploy was PR #141's `752c573`). PR #143's merge will trigger a fresh prod build that includes both. Manual fallback: `vercel deploy --prod` from `apps/studio/`.

...
### Existing files under `backend/services/execution/`

| File | Current responsibility |
|---|---|
| `backend/services/execution/order_manager.py` | Single execution path. `OrderManager.preview` and `OrderManager.submit` orchestrate `RiskGate.check`, `PreTradeValidator.validate`, broker preview/place via `broker_router`, slippage capture, and circuit-breaker fill recording. |
| `backend/services/execution/risk_gate.py` | Stateless pre-trade risk checker. Holds `MAX_ORDER_VALUE`, `MAX_SINGLE_POSITION_PCT`, `STAGE_CAPS`, `compute_position_size` (Stage Analysis Section 9 sizing formula), and `RiskViolation`. |
| `backend/services/execution/exit_cascade.py` | 9-tier long exit cascade (`_tier1_stop_loss` ... `_tier9_r5_full_exit`) plus 4-tier short exit cascade (`_short_s1_stage_improvement` ... `_short_s4_target`). `evaluate_exit_cascade` selects the highest-urgency action. |
| `backend/services/execution/broker_base.py` | Protocol for brokers. Defines `ActionSide`, `IBOrderType`, `ORDER_TYPE_MAP`, `OrderRequest`, `OrderResult`, `PreviewResult`, and `BrokerExecutor`. |
| `backend/services/execution/broker_router.py` | Resolves broker name to executor instance (`broker_router.get(broker_type)`). |
| `backend/services/execution/ibkr_executor.py` | IBKR live executor implementing `BrokerExecutor`. |
| `backend/services/execution/alpaca_executor.py` | Alpaca executor implementing `BrokerExecutor`. |
| `backend/services/execution/paper_executor.py` | Paper-trading executor used when `ALLOW_LIVE_ORDERS` is false. |
| `backend/services/execution/broker_adapter.py` | Compatibility layer between legacy callers and the `BrokerExecutor` protocol. |
| `backend/services/execution/approval_service.py` | Trade approval workflow. Defines `ApprovalMode` (`ALL`, `THRESHOLD`, `ANALYST_ONLY`, `NONE`) and `ApprovalService.requires_approval`. |
| `backend/services/execution/slippage_tracker.py` | Per-symbol, per-strategy, per-venue slippage rollups consumed by execution analytics. |
| `backend/services/execution/__init__.py` | Public surface for the package. |

### Existing files under `backend/services/risk/`

| File | Current responsibility |
|---|---|
| `backend/services/risk/circuit_breaker.py` | Module-level `circuit_breaker = CircuitBreaker()` singleton with Redis-backed tiered state (`tier1_loss_pct=2.0`, `tier2_loss_pct=3.0`, `tier3_loss_pct=5.0`), trading-day reset, kill switch, and `record_fill` accounting. State keys are global (`circuit:daily_pnl`, `circuit:order_count`, `circuit:kill_switch`, etc.). |
| `backend/services/risk/pre_trade_validator.py` | Aggregate `PreTradeValidator` running circuit breaker, position limit, sector concentration, price collar, order rate limit, wash-sale warning, and stop-distance portfolio heat (`MAX_PORTFOLIO_HEAT_PCT = 0.06`). |
| `backend/services/risk/__init__.py` | Public surface for the package. |
...
From `backend/services/execution/broker_base.py`:

```30:59:backend/services/execution/broker_base.py
...
```10:27:backend/services/execution/broker_base.py
...
```python
# new in backend/services/execution/intent.py
...
### Module: `backend/services/execution/rebalance_engine.py` (NEW)
...
`backend/services/portfolio/tax_loss_harvester.py` already implements wash-sale tracking, substantially-identical mapping, and per-position evaluation (`HarvestOpportunity`). What is missing is **lot selection on the sell path**.

Add a new module `backend/services/portfolio/lot_selector.py`:
...
New columns on `backend/models/order.py` (separate Alembic migration; reserve revision number when implementing - do **not** pick now):
...
### Module: `backend/services/strategy/signal_to_order.py` (NEW)
...
4. **Regime gate**: if `pick.tier_label not in REGIME_LONG_ACCESS[regime_state]` (from `backend/services/market/scan_engine.py`) -> reject with reason `regime_blocked`. R5 always blocks longs.
...
`backend/services/risk/pre_trade_validator.py` already enforces a **stop-distance heat cap**:

```312:394:backend/services/risk/pre_trade_validator.py
...
### Module: `backend/services/risk/portfolio_heat_guard.py` (NEW)
...
```52:76:backend/services/risk/circuit_breaker.py
...
```384:386:backend/services/risk/circuit_breaker.py
...
### New module: `backend/services/execution/bracket_manager.py` (NEW)
...
### Proposed module: `backend/services/execution/trailing_stop_manager.py` (NEW)
...
- `OrderIntent` and `IntentKind` defined in new `backend/services/execution/intent.py`.
...
- New `PortfolioHeatGuard` lives in `backend/services/risk/`.
...
- `BracketManager` lives in `backend/services/execution/`.
...
- `TrailingStopManager` lives in `backend/services/execution/`.
...
- `backend/services/execution/intent.py`
- `backend/services/execution/rebalance_engine.py`
- `backend/services/execution/bracket_manager.py`
- `backend/services/execution/trailing_stop_manager.py`
- `backend/services/portfolio/lot_selector.py`
- `backend/services/risk/portfolio_heat_guard.py`
- `backend/services/strategy/signal_to_order.py`
- `backend/models/circuit_breaker_state.py`
- `backend/models/bracket_group.py`
- `backend/models/trailing_stop_state.py`
- `backend/alembic/versions/<NEXT_REVISION>_add_circuit_breaker_state.py`
- `backend/alembic/versions/<NEXT_REVISION>_add_lot_selection_to_orders.py`
- `backend/alembic/versions/<NEXT_REVISION>_add_bracket_groups.py`
- `backend/alembic/versions/<NEXT_REVISION>_add_trailing_stop_state.py`
- `backend/api/routes/rebalance.py`
- (test files matching every fixture named above)

### Edited files (DANGER ZONE — founder line-by-line review required)

- `backend/services/execution/order_manager.py` — add `place_intent`, wire `LotSelector` into SELL path.
- `backend/services/execution/risk_gate.py` — add `allow_reducing` keyword on `check`.
- `backend/services/risk/circuit_breaker.py` — refactor module singleton into `get_circuit_breaker(user_id)` factory; per-user Redis keys.
- `backend/services/risk/pre_trade_validator.py` — add `_check_portfolio_heat_exposure` and switch to per-user breaker.
- `backend/models/order.py` — add `lot_method`, `selected_lot_ids`, `estimated_realized_gain`, `allow_wash_sale_override` columns.
- `backend/services/execution/ibkr_executor.py` / `alpaca_executor.py` / TastyTrade / Schwab executors — expose `capabilities` and bracket payload helpers.
- `backend/api/routes/orders.py` — add `place_intent` endpoint.

### Untouched

Every other file, including the entire `backend/services/market/` indicator pipeline, regime engine, scan engine, and exit cascade tier definitions. Exit cascade only **gains a consumer** (`TrailingStopManager`), it does not change behavior.

...
- **Context**: Every LaunchFree LLC former who selects "partnership" or "S-Corp" tax election MUST file a business return (Form 1065 or 1120-S). This is a mandatory compliance obligation, not an optional cross-sell. Business returns are also high-value for FileFree Pro CPA firms. No free filing product currently covers business returns -- Taxu.io charges $10-29/mo for business features, TurboTax Business starts at $219.
- **Decision**: Add business tax filing as Phase 10 (Year 2, 2027-2028 season). Forms 1065 (partnership/multi-member LLC) and 1120-S (S-Corp). Price: $49/return (1065), $99/return (1120-S) for consumer. Included in FileFree Pro Firm plan. LaunchFree cross-sell: first business return free for LLC formers who selected partnership/S-Corp election. K-1 generation for pass-through income allocation. MeF business return schemas extend existing validation engine.
...
### D69 — FileFree Pro: B2B CPA Automation as Immediate Revenue (2026-03-16)
...
- **Decision**: Add "FileFree Pro" as a B2B SaaS product (Section 1C, Phase 9). CPA firms upload client W-2s/1099s in bulk, auto-extract fields via shared OCR pipeline, export to professional tax software (UltraTax, Drake, ProConnect, Lacerte). Pricing: $49/mo (Solo), $99/mo (Team), $199/mo (Firm). Architecture: new `apps/filefree-pro/` in monorepo at `pro.filefree.ai`, sharing `packages/data`, `packages/ui`, `packages/auth` with consumer FileFree. Same `apis/filefree/` backend with firm-scoped B2B routes.
- **Alternatives**: API-only (Tax-as-a-Service for platforms -- different audience, included separately in growth playbook), skip B2B and focus on consumer marketplace only (rejected: slower to revenue, wastes shared tech opportunity), license OCR pipeline to MagneticTax or similar (rejected: creates competitor dependency, undermines own B2B offering).
- **Reversibility**: High. FileFree Pro is a separate app in the monorepo. Can be shut down without affecting consumer FileFree. Stripe subscriptions can be cancelled. No data coupling beyond shared OCR pipeline.

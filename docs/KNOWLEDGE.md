...
Architectural decisions with rationale. Grouped by domain; within each decision table, rows are sorted by decision ID. Decision IDs are globally unique through **D112** (V1 Greenfield implementation rows **D95–D102** were renumbered from a duplicate D81–D88 block that collided with Strategic Direction **D81–D88**). Decision IDs are append-only — never reused. Resolved issues use R-prefixed IDs.

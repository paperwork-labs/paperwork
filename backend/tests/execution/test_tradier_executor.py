...
def patched_decrypt():
    ...
    with patch(
        "backend.services.execution.tradier_executor.decrypt",
...
def noop_ensure_token():
    ...
    with patch(
        "backend.services.execution.tradier_executor.ensure_broker_token",
        return_value=None,
    ) as m:
        yield m
...
class TestTokenRefresh:
    ...
    def test_expired_token_is_refreshed_before_place(
        ...
        # contract in ``backend.tasks.portfolio.oauth_token_refresh``.
        ...
        with patch(
            "backend.services.market.market_data_service.infra"
        ) as mock_infra, patch(
            "backend.tasks.portfolio.oauth_token_refresh._refresh_one",

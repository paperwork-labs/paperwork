...
from backend.tasks.ops import ibkr_watchdog
...
def test_proceeds_when_account_present_but_handles_disconnect():
    ...
    with patch.object(
        ...
        "backend.services.clients.ibkr_client.ibkr_client", _FakeIbkr()
    ), patch(
        "backend.services.notifications.order_notifications.send_risk_alert",
        return_value=None,
    ):
        result = ibkr_watchdog._perform_ping()
...
def test_account_lookup_swallows_db_failure_and_returns_false():
    ...
    with patch(
        "backend.database.SessionLocal", side_effect=RuntimeError("db down")
    ):

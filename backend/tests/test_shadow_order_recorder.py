...
try:
    ...
    from backend.models.order import Order, OrderSide, OrderStatus, OrderType
    ...
    from backend.services.execution.shadow_mark_to_market import (
        mark_open_shadow_orders,
    )
    from backend.services.execution.shadow_order_recorder import (
        ShadowOrderRecorder,
    )
...
def _make_preview_order(
    db_session, *, user: "User", symbol: str, qty: float = 10.0
) -> "Order":
    order = Order(
        symbol=symbol.upper(),
...
def test_happy_path_records_executed_shadow_order(db_session, user_a):
    ...
    result = ShadowOrderRecorder(session=db_session).record(
    ...
    row = (
        ...
        .filter(ShadowOrder.id == result["shadow_order_id"])
        .one()
    )
...
def test_risk_gate_denied_order_still_persists_with_verdict(db_session, user_a):
    ...
    order = _make_preview_order(
        db_session, user=user_a, symbol="NOPRICE", qty=5
    )
...
def test_cross_tenant_isolation(db_session, user_a, user_b):
    ...
    result = ShadowOrderRecorder(session=db_session).record(
        order_id=a_order.id, user_id=user_b.id
    )
    assert result == {"error": "Order not found"}

    b_shadows = (
        db_session.query(ShadowOrder)
        .filter(ShadowOrder.user_id == user_b.id)
        .all()
    )
    assert b_shadows == []
...
def test_mark_to_market_uses_frozen_snapshot_price(db_session, user_a):
    ...
    _seed_snapshot(
        db_session, symbol="ACME", price=105.0, analysis_timestamp=later
    )
...
def test_flag_off_routes_to_real_submit_path(db_session, user_a, monkeypatch):
    ...
    result = asyncio.run(
        OrderManager().submit(db_session, order_id=order.id, user_id=user_a.id)
    )
    ...
    assert (
        db_session.query(ShadowOrder)
        .filter(ShadowOrder.user_id == user_a.id)
        .count()
        == 0
    )
...
def test_order_manager_flag_gate_exists_exactly_once() -> None:
    ...
    diff against ``backend/services/execution/order_manager.py`` before
    merging.
    """
    path = (
...
def test_shadow_mtm_celery_task_is_registered() -> None:
    ...
    importlib.import_module("backend.services.execution.shadow_mark_to_market")
    from backend.tasks.celery_app import celery_app

    assert "backend.services.execution.shadow_mark_to_market.run" in celery_app.tasks

...
from backend.tasks.celery_app import _build_beat_schedule
from backend.tasks.job_catalog import CATALOG, JobTemplate
...
def test_compute_daily_regime_template_exists_and_is_wired_correctly():
    ...
    # backend/tasks/market/regime.py — auto-generated as
    # `<module>.<function>` because no explicit name= is set.
    assert job.task == "backend.tasks.market.regime.compute_daily"
...
def test_compute_daily_regime_lands_in_beat_schedule_with_correct_crontab():
    ...
    assert entry["task"] == "backend.tasks.market.regime.compute_daily"

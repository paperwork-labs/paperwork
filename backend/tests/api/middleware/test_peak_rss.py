...
from backend.api.middleware import peak_rss
from backend.api.middleware.peak_rss import PeakRssMiddleware, is_observability_bypass_path, should_sample_for_request_id
from backend.services.market.admin_health_service import AdminHealthService
from backend.services.observability import peak_rss_store
from backend.services.observability.peak_rss_store import get_hottest_endpoints_aggregated, ru_maxrss_raw_to_kib
...
def test_darwin_ru_maxrss_bytes_to_kib() -> None:
    with mock.patch("backend.services.observability.peak_rss_store.platform", mock.MagicMock()) as p:
        p.system = mock.Mock(return_value="Darwin")
        assert ru_maxrss_raw_to_kib(1_048_576) == 1024  # 1 MiB in bytes -> 1024 KiB

    with mock.patch("backend.services.observability.peak_rss_store.platform", mock.MagicMock()) as p2:
...
async def test_high_delta_emits_warning(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("backend.config.settings.ENABLE_PEAK_RSS_MIDDLEWARE", True, raising=False)
    ...
    with mock.patch.object(peak_rss.logger, "warning") as wmock:
        with mock.patch("backend.api.middleware.peak_rss.get_request_id", return_value="x-req-1"):
            with mock.patch("backend.api.middleware.peak_rss.should_sample_for_request_id", return_value=True):
                with mock.patch("backend.api.middleware.peak_rss.is_observability_bypass_path", return_value=False):
                    with mock.patch("backend.api.middleware.peak_rss._get_sync_redis", return_value=None):
                        with mock.patch("backend.api.middleware.peak_rss._path_template_for_metrics", return_value="/api/v1/under-test"):
                            with mock.patch("backend.services.observability.peak_rss_store.platform", mock.MagicMock()) as p_sys:
                                p_sys.system = mock.Mock(return_value="Linux")
                                with mock.patch("backend.api.middleware.peak_rss.resource.getrusage", side_effect=_getrusage):
                                    tm = mock.MagicMock()
                                    tm.get_traced_memory = mock.Mock(return_value=(0, 0))
                                    with mock.patch("backend.api.middleware.peak_rss.tracemalloc", tm):
...
def test_composite_merge_hottest_and_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("backend.config.settings.ENABLE_PEAK_RSS_MIDDLEWARE", True, raising=False)
    with mock.patch(
        "backend.services.observability.peak_rss_store.get_hottest_endpoints_aggregated",
        return_value=(
            [
                {
                    "route": "GET:/x",
    ...
    monkeypatch.setattr("backend.config.settings.ENABLE_PEAK_RSS_MIDDLEWARE", False, raising=False)
    p3: dict = {}

"""Tests for :mod:`backend.services.portfolio.plaid.client`.
...
* ``_resolve_products`` and ``_extract_plaid_error`` behave as specified
  ...
  dict, not KeyError).
* Token encryption is a pass-through to :mod:`backend.services.oauth.encryption`
...
try:
    from backend.services.portfolio.plaid import client as plaid_client_module

"""Tests for ``backend.services.market.options_chain_service``.
...
from backend.services.market import options_chain_service as svc
...
class TestGetChain:
    ...
    async def test_ibkr_success_short_circuits_yfinance(self):
        ...
        with patch.dict(
            "sys.modules",
            {
                "backend.services.clients.ibkr_client": MagicMock(
                    IBKR_AVAILABLE=True, ibkr_client=fake_client
                ),
                "backend.services.market.yfinance_options_chain": MagicMock(

...
* :class:`ETradeBronzeClient` — thin wrapper around the signed-request
  helper living in ``backend.services.oauth.etrade.ETradeSandboxAdapter``.
...
* :class:`ETradeSyncService` — mirrors
  ``backend.services.portfolio.schwab_sync_service.SchwabSyncService``:
...
Tokens live in ``backend/models/broker_oauth_connection.py`` (not
...
from backend.services.bronze.etrade.sync_service import ETradeSyncService

"""Order execution service with risk guardrails."""
...
# Risk guardrails
MAX_ORDER_VALUE = 100_000  # max notional per order
MAX_SINGLE_POSITION_PCT = 0.25  # max 25% of portfolio
...
class OrderService:
    ...
    async def preview_order(
        ...
        """Create a preview order using whatIfOrder. Returns preview data + risk checks."""
        # Map order types
        ...
        # Risk checks
        warnings = []
        price_for_estimate = limit_price or stop_price or 0
        if not price_for_estimate:
            snap = (
            ...
            if snap and snap[0] is not None:
                try:
                    price_for_estimate = float(snap[0])
                except (TypeError, ValueError):
                    pass
        est_value = quantity * price_for_estimate
        if est_value > MAX_ORDER_VALUE:
            warnings.append(f"Order value ${est_value:,.0f} exceeds ${MAX_ORDER_VALUE:,.0f} guardrail")

        # Persist preview order
        order = Order(
            symbol=symbol.upper(),
            side=side.lower(),

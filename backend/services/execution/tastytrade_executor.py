...
plus a matching adapter under ``backend.services.oauth.<broker>``.
...
* There is no ``backend.services.oauth.tastytrade`` adapter; the session
...
class TastytradeExecutor:
    ...
    (``backend.services.clients.TastyTradeClient``) but is not a
    ...
    :class:`TastyTradeClient` in ``backend.services.clients``). Migrating to
...
def _try_get_redis() -> Optional[Any]:
    ...
    try:
        ...
        logger.warning("tastytrade: backend.services.cache redis unavailable: %s", exc)
        return None

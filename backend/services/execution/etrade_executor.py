...
:mod:`backend.services.oauth.etrade`. We do **not** re-implement the
...
* Every write path calls :func:`backend.services.execution.oauth_executor_mixin.ensure_broker_token`
...
from backend.models.broker_oauth_connection import BrokerOAuthConnection
...
from backend.services.oauth.encryption import decrypt
from backend.services.oauth.etrade import (
    _build_oauth_header,
    _nonce,
    _timestamp,
...
ConnectionResolver = Callable[
    [OrderRequest], Tuple[Session, BrokerOAuthConnection, str]
]
...
class ETradeExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        if env not in ("sandbox", "prod"):
            raise ValueError(
                f"ETradeExecutor environment must be 'sandbox' or 'prod', "
                f"got {environment!r}"
            )
        if env == "prod" and not bool(getattr(settings, "ETRADE_ALLOW_LIVE", False)):
            raise RuntimeError(
                "E*TRADE live orders disabled; set ETRADE_ALLOW_LIVE=true"
            )
        ...
        self._consumer_key = (
            consumer_key
            or getattr(settings, "ETRADE_SANDBOX_KEY", None)
        )
        self._consumer_secret = (
            consumer_secret
            or getattr(settings, "ETRADE_SANDBOX_SECRET", None)
        )
    ...
    async def preview_order(self, req: OrderRequest) -> PreviewResult:
        ...
        try:
            body = await self._signed_json_request(
                "POST", path, conn, json_body=payload
            )
    ...
    async def place_order(self, req: OrderRequest) -> OrderResult:
        ...
        try:
            ...
            logger.warning(
                ...
                exc.status, preview_path, exc.detail,
            )
            return OrderResult(
                error=f"E*TRADE preview failed: HTTP {exc.status} {exc.detail[:200]}",
        ...
        try:
            ...
            logger.warning(
                ...
                exc.status, place_path, exc.detail,
            )
            return OrderResult(
    ...
    async def cancel_order(self, broker_order_id: str) -> OrderResult:
        ...
        try:
            body = await self._signed_json_request(
                "PUT", path, conn, json_body=payload
            )
        except _ETradeHTTPError as exc:
            logger.warning(
                "E*TRADE cancel HTTP %s on %s: %s",
                exc.status, path, exc.detail,
            )
            return OrderResult(
    ...
    def _require_context(
        ...
        try:
            ...
            raise RuntimeError(
                f"ETradeExecutor connection_resolver raised: {exc}"
            ) from exc
    ...
    def _build_auth_header(
        ...
        *,
        ...
        params["oauth_signature"] = sign_hmac_sha1(
            base, self._consumer_secret, token_secret
        )
    ...
    def _decrypt_tokens(self, conn: BrokerOAuthConnection) -> Tuple[str, str]:
        ...
        try:
            ...
            raise RuntimeError(
                f"E*TRADE connection {conn.id} token decrypt failed: {exc}"
            ) from exc
        if not access or not secret:
            raise RuntimeError(
                f"E*TRADE connection {conn.id} decrypted token is empty"
            )
...
def _validate_context(
    ...
    if not isinstance(ctx, tuple) or len(ctx) != 3:
        raise RuntimeError(
            "connection_resolver must return (Session, BrokerOAuthConnection, "
            "account_id_key)"
        )
...
def _extract_preview_id(body: Dict[str, Any]) -> Optional[int]:
...
def _extract_order_id(body: Dict[str, Any]) -> Optional[Any]:

...
PIPELINE_DAG: Dict[str, StepDef] = {
    "constituents": StepDef(
        deps=(),
        task_path="backend.tasks.market.backfill.constituents",
        timeout_s=300,
        display_name="Index Constituents",
    ),
    "tracked_cache": StepDef(
        deps=("constituents",),
        task_path="backend.tasks.market.backfill.tracked_cache",
        timeout_s=120,
    ...
    "indicators": StepDef(
        deps=("daily_bars",),
        task_path="backend.tasks.market.indicators.recompute_universe",
        timeout_s=3600,
        display_name="Indicators + Stages",
    ),
    "regime": StepDef(
        deps=("daily_bars",),
        task_path="backend.tasks.market.regime.compute_daily",
        timeout_s=600,
        display_name="Market Regime",
    ),
    "scan_overlay": StepDef(
        deps=("indicators", "regime"),
        task_path="backend.tasks.market.coverage._run_scan_overlay",
        timeout_s=600,
        display_name="Scan Overlay",
    ),
    "snapshot_history": StepDef(
        deps=("indicators",),
        task_path="backend.tasks.market.history.snapshot_last_n_days",
        timeout_s=1800,
        display_name="Snapshot History",
    ),
    "exit_cascade": StepDef(
        deps=("indicators",),
        task_path="backend.tasks.market.coverage._evaluate_exit_cascade_all",
        timeout_s=600,
        display_name="Exit Cascade",
    ),
    "strategy_eval": StepDef(
        deps=("indicators", "regime"),
        task_path="backend.tasks.strategy.tasks.evaluate_strategies_task",
        timeout_s=660,
        display_name="Strategy Evaluation",
    ),
    "health_check": StepDef(
        deps=("indicators",),
        task_path="backend.tasks.market.coverage.health_check",
        timeout_s=180,
    ...
    "digest": StepDef(
        ...
        task_path="backend.tasks.intelligence.tasks.generate_daily_digest_task",
        timeout_s=600,
...
_CELERY_NAME_OVERRIDES: Dict[str, str] = {
    "backend.tasks.strategy_tasks.evaluate_strategies_task": "strategy_eval",
    "backend.tasks.intelligence_tasks.generate_daily_digest": "digest",
}

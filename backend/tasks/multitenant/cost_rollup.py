...
from backend.services.multitenant.cost_attribution import CostAttributionService

logger = logging.getLogger(__name__)


@shared_task(
    name="backend.tasks.multitenant.cost_rollup.rollup_yesterday",
    queue="celery",
    soft_time_limit=240,
    time_limit=300,
)
...
@shared_task(
    name="backend.tasks.multitenant.cost_rollup.rollup_day",
    queue="celery",
    soft_time_limit=240,
    time_limit=300,
)
